...
def generate_enhanced_export(theorem_data, progressive_explanation):
    ...
    exploration_content = "<ul>" + "".join(f"<li>{item}</li>" for item in progressive_explanation.exploration_suggestions[:5]) + "</ul>"
    visualization_content = "<ul>" + "".join(f"<li>{item}</li>" for item in progressive_explanation.visualization_ideas[:5]) + "</ul>"
    extension_content = "<ul>" + "".join(f"<li>{item}</li>" for item in progressive_explanation.extension_problems[:5]) + "</ul>"
    examples_content = "<ul>" + "".join(f"<li>{item}</li>" for item in progressive_explanation.intuitive_examples[:5]) + "</ul>"
    
    # Calculate statistics
    total_steps = sum(len(sketch.key_steps) for sketch in progressive_explanation.levels.values())
    
    # Fill in the template
    html_content = template.replace("{{title}}", theorem_data["title"])
    html_content = html_content.replace("{{description}}", "An interactive educational exploration of this mathematical theorem")
    html_content = html_content.replace("{{audience}}", theorem_data["audience"])
    html_content = html_content.replace("{{level_count}}", str(len(progressive_explanation.levels)))
    html_content = html_content.replace("{{step_count}}", str(total_steps))
    html_content = html_content.replace("{{concept_count}}", str(len(progressive_explanation.key_concepts)))
    html_content = html_content.replace("{{complexity_score}}", f"{progressive_explanation.difficulty_progression[0]:.1f}")
    ...
    html_content = html_content.replace("{{visualization_content}}", visualization_content)
...
<body>
    <div class="container">
        ...
        <div class="stats">
            <div>
                <div class="stat-number">""" + str(len([r for r in demo_results if r.get('success')])) + """</div>
...
"""
    ...
    if successful_demos:
        avg_parse_time = sum(r['parse_time'] for r in successful_demos) / len(successful_demos)
        avg_gen_time = sum(r['generation_time'] for r in successful_demos) / len(successful_demos)
