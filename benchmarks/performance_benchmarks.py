...
class PerformanceBenchmark:
    ...
    async def run_all_benchmarks(self) -> BenchmarkSuite:
        ...
        console.print("[bold blue]🚀 Starting Performance Benchmark Suite[/bold blue]\n")
    ...
    async def _run_single_benchmark(self, test_name: str, theorem_count: int, 
        ...
        try:
            ...
            try:
                from src.proof_sketcher.exporter.models import ExportContext, ExportFormat
    ...
    def _calculate_summary_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_theorems_processed": sum(r.theorem_count for r in successful_results),
            "average_theorems_per_second": statistics.mean(r.theorems_per_second for r in successful_results),
            "median_theorems_per_second": statistics.median(r.theorems_per_second for r in successful_results),
            "peak_memory_usage_mb": max(r.peak_memory_mb for r in successful_results),
            "average_parse_time_per_theorem_ms": statistics.mean(r.parse_time_per_theorem_ms for r in successful_results),
            "average_generation_time_per_theorem_ms": statistics.mean(r.generation_time_per_theorem_ms for r in successful_results),
            "scaling_factors": self._calculate_scaling_factors(successful_results)
        }
    
    def _calculate_scaling_factors(self, results: List[BenchmarkResult]) -> Dict[str, float]:
        ...
        return {
            ...
            "time_scaling_factor": time_ratio / theorem_ratio,  # How much slower per theorem
            "memory_scaling_factor": memory_ratio / theorem_ratio,  # How much more memory per theorem
            "efficiency_degradation": (time_ratio / theorem_ratio) - 1.0  # % performance loss per theorem
        }
    ...
    def display_results(self, suite: BenchmarkSuite) -> None:
        ...
        if "error" not in suite.summary_stats:
            ...
            console.print(f"  • Total theorems processed: {stats['total_theorems_processed']}")
            console.print(f"  • Average processing rate: {stats['average_theorems_per_second']:.1f} theorems/sec")
            console.print(f"  • Peak memory usage: {stats['peak_memory_usage_mb']:.1f} MB")
            console.print(f"  • Successful benchmarks: {stats['successful_benchmarks']}/{stats['total_benchmarks']}")
            
            if "scaling_factors" in stats:
                scaling = stats["scaling_factors"]
                console.print(f"\n[bold yellow]⚡ Scaling Analysis[/bold yellow]")
                console.print(f"  • Time scaling factor: {scaling.get('time_scaling_factor', 0):.2f}x per theorem")
                console.print(f"  • Memory scaling factor: {scaling.get('memory_scaling_factor', 0):.2f}x per theorem")
                console.print(f"  • Efficiency degradation: {scaling.get('efficiency_degradation', 0)*100:.1f}% per theorem")
    ...
    async def run_benchmarks():
        ...
        try:
            ...
            if successful_results:
                ...
                if avg_rate < 1.0:
                    console.print("  ⚠️  Low processing rate - consider optimizing parsing or generation")
                elif avg_rate > 10.0:
                    console.print("  ✅ Excellent processing rate - suitable for large-scale processing")
                
                if peak_memory > 1000:
                    console.print("  ⚠️  High memory usage - consider batch processing for large files")
                elif peak_memory < 100:
                    console.print("  ✅ Efficient memory usage - suitable for resource-constrained environments")
                ...
                if efficiency_loss > 0.5:
                    console.print("  ⚠️  Significant efficiency degradation with file size - investigate algorithmic complexity")
                elif efficiency_loss < 0.1:
                    console.print("  ✅ Good scaling characteristics - linear performance with file size")
