...
from proof_sketcher.parser.mathlib_notation_optimized import OptimizedMathlibNotationHandler, create_optimized_handler
...
def demo_notation_optimization():
    ...
    optimized_handler = create_optimized_handler(
        enable_caching=True,
        expected_dataset_size=len(test_expressions)
    )
    ...
    print(f"   Memory Efficiency: {stats['memory_efficiency']['interned_symbols']} interned symbols")
    ...
    print(f"   Memory Savings: {stats['memory_efficiency']['interned_symbols']} objects interned")
...
async def demo_smart_caching():
    ...
    print(f"   Hit Rate: {len([r for r in hit_results if r is not None])/len(hit_results)*100:.1f}%")
...
def demo_resource_management():
    ...
    print(f"   Available Memory: {report['system_resources']['available_memory_gb']:.1f} GB")
...
async def demo_batch_optimization():
    ...
    print(f"   Deduplicated: {stats['deduplicated_requests']} ({stats['deduplicated_requests_percent']:.1f}%)")
...
async def main():
    ...
    if 'notation_optimization' in all_results:
        ...
        print(f"   • Notation Handler: {notation['speedup']:.2f}x faster (batch: {notation['batch_speedup']:.2f}x)")
    ...
    if 'batch_optimization' in all_results:
        ...
        print(f"   • Batch Processing: {batch['stats']['deduplicated_requests_percent']:.1f}% deduplication")
