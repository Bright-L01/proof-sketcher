...
from typing import Any, Dict, List, Optional, AsyncGenerator, Union, TextIO
...
from ..exporter.models import ExportFormat, ExportContext
...
class StreamingExporter:
        ...
        try:
            with writer:
                ...
                total_stats.average_throughput_mbps = (
                    (total_stats.total_bytes_written / 1024 / 1024) / total_stats.total_time
                    if total_stats.total_time > 0 else 0
                )
    ...
    async def export_with_memory_limit(
        ...
        for i in range(0, len(sketches), batch_size):
            ...
            # Export batch
            batch_output = output_path.with_name(f"{output_path.stem}_batch_{i // batch_size}{output_path.suffix}")
    ...
    async def _combine_batch_files(self, output_path: Path, batch_stats: List[StreamStats]):
        """Combine batch files into single output."""
        # This would depend on the specific format
        # For now, just log that batching occurred
        self.logger.info(f"Exported in {len(batch_stats)} batches, combination may be needed")
...
def estimate_memory_usage(sketches: List[ProofSketch]) -> int:
    ...
    for sketch in sketches:
        ...
        total_chars += sum(len(step.mathematical_content or "") for step in sketch.key_steps)
...
def should_use_streaming(sketches: List[ProofSketch], memory_limit_mb: int = 100) -> bool:
