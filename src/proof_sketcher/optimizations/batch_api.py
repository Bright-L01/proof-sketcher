...
class BatchAPIOptimizer:
    ...
    async def _process_batch(self, batch: List[BatchRequest]):
        ...
        for method, requests in method_groups.items():
            task = asyncio.create_task(
                self._process_method_group(method, requests)
            )
            tasks.append(task)
    ...
    async def _make_api_call(self, method: str, requests: List[BatchRequest]) -> List[Any]:
    ...
    async def _handle_response(self, request: BatchRequest, response: BatchResponse):
        ...
        if response.success and self.config.enable_caching:
            cache_key = f"batch_api:{request.method}:{self._hash_params(request.params)}"
        ...
        if request.id in self.request_hashes:
            ...
            if request_hash in self.hash_to_requests:
                self.hash_to_requests[request_hash] = [
                    r for r in self.hash_to_requests[request_hash]
                    if r.id != request.id
                ]
        ...
        stats = {
            **self.stats,
            ...
            'success_rate_percent': (
                ((total_requests - self.stats['failed_requests']) / total_requests) * 100
                if total_requests > 0 else 0
            ),
            'avg_processing_time': (
                self.stats['total_processing_time'] / max(1, self.stats['batched_requests'])
            )
        }
