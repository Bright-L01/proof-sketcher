...
class ResourceMonitor:
    ...
    def _check_limits(self, usage: ResourceUsage):
        ...
        if usage.memory_mb > self.limits.emergency_cleanup_threshold_mb:
            self.logger.critical(f"Emergency memory cleanup triggered at {usage.memory_mb:.1f}MB")
...
class ResourceLimiter:
    ...
    def limit_animation_complexity(self, theorem: Dict[str, Any]) -> Dict[str, Any]:
        ...
        config['max_duration'] = min(
            config['max_duration'],
            self.limits.max_animation_duration
        )
    ...
    def get_resource_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'status': {
                ...
                'within_limits': (
                    current_usage.memory_mb < self.limits.max_memory_mb and
                    current_usage.cpu_percent < self.limits.max_cpu_percent
                )
            }
        }
