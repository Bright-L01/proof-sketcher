...
class SafeSerializer:
            ...
            if isinstance(data, dict):
                for key, value in data.items():
                    if not isinstance(key, (str, int)):
                        raise SecurityError(
                            f"Unsafe dict key type at {path}.{key}: {
                                type(key)}"
                        )
    ...
    def _convert_to_safe(data: Any) -> Any:
        ...
        if isinstance(data, SAFE_TYPES):
            if isinstance(data, dict):
                return {
                    str(k): SafeSerializer._convert_to_safe(v) for k, v in data.items()
                }
            elif isinstance(data, list):
            ...
            obj_data = {
                ...
                "__data__": {
                    k: SafeSerializer._convert_to_safe(v)
    ...
    def _convert_from_safe(data: Any) -> Any:
        ...
        if isinstance(data, dict):
            ...
            if "__datetime__" in data:
                ...
                return {
                    k: SafeSerializer._convert_from_safe(v) for k, v in data.items()
                }
        elif isinstance(data, list):
...
class LegacyPickleConverter:
    ...
    def convert_pickle_to_json(
        pickle_path: Path, json_path: Path, compress: bool = False
    ) -> bool:
        """
