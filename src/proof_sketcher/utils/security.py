...
def validate_lean_code(code: str) -> str:
    ...
    if len(code) > max_size:
        raise InvalidInputError(
            f"Lean code too large: {len(code)} > {max_size}"
        )
    ...
    class RestrictedUnpickler(pickle.Unpickler):
        ...
        def find_class(self, module, name):
            ...
            if module not in ALLOWED_MODULES:
                raise SecurityError(
                    f"Unpickling objects from {module} is not allowed"
                )
...
def validate_url(url: str) -> str:
    ...
    if hostname in ('localhost', '127.0.0.1', '0.0.0.0'):  # nosec B104 - Intentionally blocking these
