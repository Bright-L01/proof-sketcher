...
class EnhancedLeanParser:
    ...
    ) -> tuple[Optional[LeanDeclaration], int]:
        """Parse a specific construct based on its type."""

        if construct_name in ["theorem", "lemma"]:
            return self._parse_theorem(
                match, lines, start_line, docstring, attributes, visibility
            )
    ...
    def _parse_theorem(
        ...
        attributes: List[LeanAttribute],
        visibility: str,
    ) -> tuple[LeanTheorem, int]:
        """Parse theorem or lemma declaration."""
        construct_type = (
            LeanConstruct.THEOREM
            if match.group(1) == "theorem"
            else LeanConstruct.LEMMA
        )
        name = match.group(2)
    ...
    def _parse_inductive(
        ...
        attributes: List[LeanAttribute],
        visibility: str,
    ) -> tuple[LeanInductive, int]:
        """Parse inductive type declaration."""
        name = match.group(1)

        # Extract type signature and constructors
        type_sig, constructors, derives, end_line = self._extract_inductive_content(
            lines, start_line
        )
    ...
    def _parse_structure(
        ...
        attributes: List[LeanAttribute],
        visibility: str,
    ) -> tuple[LeanStructure, int]:
        """Parse structure declaration."""
        name = match.group(1)

        # Extract parameters, extends, and fields
        parameters, extends, fields, derives, end_line = (
            self._extract_structure_content(lines, start_line)
        )
    ...
    def _parse_class(
        ...
        parameters, extends, fields, _, end_line = self._extract_structure_content(
            lines, start_line
        )
    ...
    def _parse_instance(
        ...
        class_name, instance_type, parameters, proof, end_line = (
            self._extract_instance_content(lines, start_line)
        )
    ...
    def _parse_axiom(
        ...
        statement, _, end_line = self._extract_theorem_content(
            lines, start_line, expect_proof=False
        )
    ...
    def _extract_theorem_content(
        self, lines: List[str], start_line: int, expect_proof: bool = True
        ...
        if in_proof and expect_proof:
            ...
            while i < len(lines):
                ...
                if brace_count == 0 and (
                    line.endswith("qed")
                    or line.endswith("sorry")
                    or any(
                        keyword in line
                        for keyword in ["theorem ", "lemma ", "def ", "end "]
                    )
                ):
                    break
    ...
    def _extract_definition_content(
        self, lines: List[str], start_line: int
    ) -> tuple[str, Optional[str], int]:
    ...
    def _extract_inductive_content(
        self, lines: List[str], start_line: int
    ) -> tuple[str, List[Dict[str, str]], List[str], int]:
        ...
        while i < len(lines) and not lines[i].strip().startswith("end"):
            ...
            if line.startswith("|"):
                ...
                constructors.append(
                    {"name": constructor_name, "type": constructor_type}
                )
    ...
    def _extract_structure_content(
        self, lines: List[str], start_line: int
    ) -> tuple[List[str], List[str], List[Dict[str, str]], List[str], int]:
        ...
        while i < len(lines) and not any(
            keyword in lines[i] for keyword in ["end", "theorem", "lemma", "de"]
        ):
            line = lines[i].strip()
            if "extends" in line:
    ...
    def _extract_instance_content(
        self, lines: List[str], start_line: int
    ) -> tuple[str, str, List[str], Optional[str], int]:
        ...
        _, proof, end_line = self._extract_theorem_content(
            lines, start_line, expect_proof=True
        )
    ...
    def get_theorems_for_proof_sketcher(
        self, declarations: Dict[str, List[LeanDeclaration]]
    ) -> List[TheoremInfo]:
