...
class MathlibNotationHandler:
    ...
    def _build_notation_mappings(self) -> None:
        ...
        self.core_symbols = [
            ...
            NotationMapping('⊆', r'\subseteq', '&sube;', 'subset or equal', 'set_theory'),
            NotationMapping('⊂', r'\subset', '&sub;', 'proper subset', 'set_theory'),
            NotationMapping('⊇', r'\supseteq', '&supe;', 'superset or equal', 'set_theory'),
        ...
        self.delimiter_symbols = [
            NotationMapping('⟨', r'\langle', '&lang;', 'left angle bracket', 'delimiters'),
            NotationMapping('⟩', r'\rangle', '&rang;', 'right angle bracket', 'delimiters'),
            NotationMapping('⟦', r'\llbracket', '&#10214;', 'left semantic bracket', 'delimiters'),
            NotationMapping('⟧', r'\rrbracket', '&#10215;', 'right semantic bracket', 'delimiters'),
        ...
        # Number systems
        self.number_systems = [
            NotationMapping('ℕ', r'\mathbb{N}', '&#8469;', 'natural numbers', 'numbers'),
            ...
            NotationMapping('ℂ', r'\mathbb{C}', '&#8450;', 'complex numbers', 'numbers'),
        ...
        self.analysis_symbols = [
            ...
            NotationMapping('∂', r'\partial', '&part;', 'partial derivative', 'analysis'),
            NotationMapping('∇', r'\nabla', '&nabla;', 'gradient', 'analysis'),
            NotationMapping('△', r'\triangle', '&#9651;', 'triangle/Laplacian', 'analysis'),
        ...
        self.category_symbols = [
            NotationMapping('⟶', r'\longrightarrow', '&#10230;', 'morphism', 'category'),
            NotationMapping('⇒', r'\Rightarrow', '&rArr;', 'natural transformation', 'category'),
            NotationMapping('⇔', r'\Leftrightarrow', '&hArr;', 'equivalence', 'category'),
        ...
        self.all_mappings = (
            self.core_symbols + self.logic_symbols + self.algebra_symbols +
            self.order_symbols + self.delimiter_symbols + self.number_systems +
            self.analysis_symbols + self.category_symbols
        )
        ...
        self.symbol_descriptions = {m.lean_symbol: m.description for m in self.all_mappings}
    ...
    def _compile_patterns(self) -> None:
        ...
        self.typeclass_pattern = re.compile(r'\[([A-Z][A-Za-z0-9]*)\s+([a-zA-Z][a-zA-Z0-9]*)\]')
        
        # Pattern for dependent types (x : α) → β
        self.dependent_type_pattern = re.compile(r'\(([a-zA-Z][a-zA-Z0-9]*)\s*:\s*([α-ωΑ-Ω][a-zA-Z0-9]*)\)\s*→')
        
        # Pattern for namespace qualification
        self.namespace_pattern = re.compile(r'([A-Z][a-zA-Z0-9]*\.)+([a-zA-Z][a-zA-Z0-9_]*)')
    ...
    def convert_to_latex(self, lean_notation: str) -> str:
        ...
        result = self.symbol_pattern.sub(
            lambda m: self.lean_to_latex.get(m.group(0), m.group(0)),
            result
        )
    ...
    def convert_to_html(self, lean_notation: str) -> str:
        ...
        result = self.symbol_pattern.sub(
            lambda m: self.lean_to_html.get(m.group(0), m.group(0)),
            result
        )
    ...
    def _clean_latex_spacing(self, text: str) -> str:
        ...
        text = re.sub(r'\s*(\\(?:to|in|notin|subseteq|subset|supseteq|supset|cup|cap|emptyset|iff|forall|exists|neg|land|lor|top|bot|times|cdot|circ|oplus|otimes|sqcup|sqcap|le|ge|ne|approx|equiv|sim|simeq|cong|sum|prod|int|partial|nabla|triangle|Delta|infty|bigsqcup|bigsqcap|longrightarrow|Rightarrow|Leftrightarrow|dashv))\s*', r' \1 ', text)
