...
class LeanExtractor:
    ...
    def __init__(self, extractor_path: Optional[Path] = None, lean_executable: str = "lean"):
    ...
    def _find_extractor(self) -> Optional[Path]:
        ...
        possible_paths = [
            Path(__file__).parent / "../../.." / "lean_extractor" / ".lake" / "build" / "bin" / "extract",
            ...
            if source_dir.exists() and (source_dir / "lakefile.lean").exists():
                try:
                    ...
                    if result.returncode == 0:
                        self.extractor = source_dir / ".lake" / "build" / "bin" / "extract"
                        if self.extractor.exists():
                            self.logger.info("Successfully built Lean extractor")
                            return
                    else:
                        self.logger.warning(f"Failed to build extractor: {result.stderr}")
    ...
    def extract_file(self, lean_file: Path, base_dir: Optional[Path] = None) -> List[Dict]:
        ...
        if not self.extractor:
            raise LeanExtractorError("Lean extractor not available. Please build it first.")
        
        try:
            # Use the extractor to extract all theorems
            if self.extractor.suffix == ".lean":
                # Run Lean script directly
                cmd = [self.lean_executable, str(self.extractor), "--", "--file", str(lean_file), "--all"]
            else:
    ...
    def extract_theorem(self, lean_file: Path, theorem_name: str, 
                       base_dir: Optional[Path] = None) -> Optional[Dict]:
        ...
        try:
            ...
            if self.extractor.suffix == ".lean":
                # Run Lean script directly
                cmd = [self.lean_executable, str(self.extractor), "--", 
                       "--file", str(lean_file), "--theorem", theorem_name]
            else:
                # Run built executable
                cmd = [str(self.extractor), "--file", str(lean_file), "--theorem", theorem_name]
    ...
    def extract_to_theorem_info(self, lean_file: Path, theorem_name: str,
                               base_dir: Optional[Path] = None) -> Optional[TheoremInfo]:
