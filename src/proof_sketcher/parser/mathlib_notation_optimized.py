...
class OptimizedMathlibNotationHandler(MathlibNotationHandler):
    ...
    def _build_fast_patterns(self) -> Dict[str, str]:
        ...
        return {
            self._intern_string(k, self.interned_symbols): 
            self._intern_string(v, self.interned_latex)
    ...
    def _compile_optimized_patterns(self):
        ...
        other_symbols = [
            s for s in self.lean_to_latex.keys() 
            if s not in fast_symbols
        ]
        ...
        for symbol in found_symbols:
            if symbol in self.lean_to_latex:
                notation_table.append({
                    ...
                    'description': self.symbol_descriptions.get(symbol, f'Symbol: {symbol}')
                })
    ...
    def get_performance_stats(self) -> Dict[str, any]:
        ...
        cache_hit_rate = (self.cache_hits / total_conversions) * 100 if total_conversions > 0 else 0
        fast_path_rate = (self.fast_path_hits / total_conversions) * 100 if total_conversions > 0 else 0
        ...
        return {
            ...
            'latex_cache_size': self.convert_to_latex_cached.cache_info().currsize if self.enable_caching else 0,
            'html_cache_size': self.convert_to_html_cached.cache_info().currsize if self.enable_caching else 0,
            'memory_efficiency': {
    ...
    def benchmark_performance(self, test_expressions: List[str], iterations: int = 1) -> Dict[str, float]:
        ...
        return {
            ...
            'latex_throughput_ops_per_second': total_ops / latex_time if latex_time > 0 else 0,
            'html_throughput_ops_per_second': total_ops / html_time if html_time > 0 else 0,
            'batch_throughput_ops_per_second': total_ops / batch_latex_time if batch_latex_time > 0 else 0,
