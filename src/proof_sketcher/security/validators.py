...
class SecurityValidator:
    ...
    def sanitize_command(cmd: str) -> str:
        ...
        if len(cmd) > SecurityValidator.MAX_COMMAND_LENGTH:
            raise SecurityError(
                f"Command too long: {len(cmd)} > {SecurityValidator.MAX_COMMAND_LENGTH}"
            )
    ...
    @staticmethod
    def validate_path(
        user_path: str, base_dir: str, allow_absolute: bool = False
    ) -> Path:
        """
        ...
        if len(parts) > SecurityValidator.MAX_PATH_DEPTH:
            raise SecurityError(
                f"Path too deep: {len(parts)} > {SecurityValidator.MAX_PATH_DEPTH}"
            )
        ...
        for part in parts:
            ...
            if len(part) > SecurityValidator.MAX_FILENAME_LENGTH:
                raise SecurityError(
                    f"Filename too long: {len(part)} > {SecurityValidator.MAX_FILENAME_LENGTH}"
                )
    ...
    def validate_theorem_name(name: str) -> str:
        ...
        if len(name) > SecurityValidator.MAX_THEOREM_NAME_LENGTH:
            raise SecurityError(
                f"Theorem name too long: {len(name)} > {SecurityValidator.MAX_THEOREM_NAME_LENGTH}"
            )
    ...
    def hash_sensitive_data(data: str, salt: str = None) -> Dict[str, str]:
        ...
        return {"hash": hash_obj.hexdigest(), "salt": salt, "algorithm": "sha256"}
    ...
    def verify_hash(data: str, hash_info: Dict[str, str]) -> bool:
        ...
        """
        computed = SecurityValidator.hash_sensitive_data(data, hash_info["salt"])
