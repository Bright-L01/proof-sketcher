...
'''
    ...
    def _generate_proof_animation(
        self, sketch: Dict[str, Any], timings: Dict[str, float]
    ) -> str:
        ...
        code_lines.append(
            "            self.statement.animate.to_corner(UR).scale(0.7),"
        )
        ...
        for i, step in enumerate(steps):
            ...
            if description:
                ...
                code_lines.append(
                    f'            "{self._escape_string(description[:80])}",'
                )
                code_lines.append(f"            font_size=24,")
                code_lines.append(f'            color={self.colors["explanation"]}')
                code_lines.append(f"        )")
                code_lines.append(
                    f"        step_{i}_desc.next_to(step_{i}_title, DOWN, aligned_edge=LEFT)"
                )
                code_lines.append("")
            ...
            if i > 0:  # Clear previous step
                code_lines.append("        self.play(")
                code_lines.append(
                    f"            *[FadeOut(mob) for mob in self.mobjects"
                )
                code_lines.append(
                    f"              if mob not in [self.title, self.statement]],"
                )
            ...
            if tactics:
                ...
                code_lines.append(
                    f'            "Tactics: {self._escape_string(tactics_str)}",'
                )
    ...
    def _escape_latex(self, text: str) -> str:
        ...
        for char, escape in latex_escapes.items():
            if char != "\\" or not any(
                cmd in result for cmd in [r"\\forall", r"\\exists", r"\\to"]
            ):
    ...
    def generate_simple_animation(
        self, theorem_name: str, statement: str, steps: Optional[List[str]] = None
    ) -> str:
