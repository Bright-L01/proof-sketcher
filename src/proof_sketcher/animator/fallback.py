...
from .models import AnimationQuality, AnimationStyle
...
class StaticAnimationGenerator:
    ...
    def __init__(self, output_dir: Optional[Path] = None):
        ...
        """
        self.output_dir = (
            output_dir or Path(tempfile.gettempdir()) / "proof_sketcher_static"
        )
    ...
    def create_title_slide(self, title: str, subtitle: str, output_path: Path) -> bool:
        ...
        try:
            ...
            draw.text(
                (subtitle_x, img_height // 2), subtitle, fill="gray", font=subtitle_font
            )
    ...
    def create_step_slide(
        self, step: ProofStep, step_num: int, total_steps: int, output_path: Path
    ) -> bool:
                ...
                if self.render_latex_to_image(step.mathematical_content, math_img_path):
                    try:
                        ...
                        if math_img.width > max_width:
                            ...
                            new_size = (
                                int(math_img.width * ratio),
                                int(math_img.height * ratio),
                            )
                            math_img = math_img.resize(
    ...
    def create_conclusion_slide(self, conclusion: str, output_path: Path) -> bool:
        ...
        try:
            ...
            # Add QED symbol
            draw.text(
                (img_width - 150, img_height - 100), "□", fill="black", font=header_font
            )
    ...
    async def generate_static_animation(
        self, proof_sketch: ProofSketch, output_filename: str = "animation.mp4"
    ) -> dict:
        ...
        try:
            ...
            if self.create_title_slide(
                proof_sketch.theorem_name, "Proof Visualization", title_path
            ):
            ...
            if self.create_step_slide(
                intro_step, 0, len(proof_sketch.key_steps) + 1, intro_path
            ):
            ...
            for i, step in enumerate(proof_sketch.key_steps):
                ...
                if self.create_step_slide(
                    step, i + 1, len(proof_sketch.key_steps), step_path
                ):
            ...
            if self.images_to_video(images, video_path):
                ...
                return {
                    ...
                    "frame_count": 1,
                    "size_bytes": (
                        static_path.stat().st_size if static_path.exists() else 0
                    ),
                    "error": None,
                    "metadata": {
                        "generator": "static",
            ...
            return {
                ...
                "size_bytes": 0,
                "error": str(e),
                "metadata": {"generator": "static", "error": str(e)},
            }
            ...
            try:
                ...
                if await self.mcp_client.health_check():
                    ...
                    if response.video_path and response.video_path.exists():
                        ...
                        self.logger.warning(
                            "Manim returned invalid response, falling back"
                        )
                else:
                ...
                self.logger.warning(
                    f"Manim animation failed: {e}, falling back to static"
                )
