...
class MockMCPServer:
    ...
    def __init__(
        self, success_rate: float = 0.8, delay_range: Tuple[float, float] = (0.1, 0.5)
    ):
    ...
    async def handle_request(
        ...
        delay = (
            secrets.randbelow(int((self.delay_range[1] - self.delay_range[0]) * 1000))
            / 1000
            + self.delay_range[0]
        )
        ...
        if secrets.randbelow(100) / 100 > self.success_rate:
            error_types = [
                ...
                {
                    "code": -32602,
                    "message": "Invalid params: Unsupported animation style",
                },
            ]
...
class MockMCPTransport:
    ...
    async def send_request(
        ...
        if "error" in response:
            return {"jsonrpc": "2.0", "error": response["error"], "id": request["id"]}
        else:
            return {
                "jsonrpc": "2.0",
...
def create_mock_animation_response(
    proof_sketch: ProofSketch, success: bool = True, video_path: Optional[Path] = None
) -> AnimationResponse:
    ...
    if success:
        return AnimationResponse(
            ...
            metadata={"mock": True, "theorem": proof_sketch.theorem_name},
        )
    else:
        return AnimationResponse(
            video_path=None,
