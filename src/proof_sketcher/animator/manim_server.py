...
from ..utils.security import validate_file_path, sanitize_filename
...
class ManimMCPServer:
    ...
    def __init__(self, temp_dir: Optional[Path] = None):
        ...
        if not self.manim_available:
            self.logger.warning("Manim not available - animations will fall back to static generation")
    ...
    def setup_handlers(self):
        ...
        async def create_animation(
            ...
            """
            return await self._create_animation_impl(script, output_path, quality, scene_name)
        ...
        try:
            ...
            try:
                return loop.run_until_complete(
                    self._create_animation_impl(script, output_path, quality, scene_name)
                )
            finally:
                loop.close()
        except Exception as e:
