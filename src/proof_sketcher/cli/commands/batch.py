...
@click.option("--exclude", multiple=True, help="Exclude patterns (e.g., '**/test/**')")
@click.option(
    "--enhanced/--basic", default=True, help="Use enhanced parser (recommended)"
)
@click.option(
    "--report",
    type=click.Path(path_type=Path),
...
def batch(
    ...
    try:
        ...
        if stats.theorems_per_second > 5.0:
            console.print(
                "  ✅ Excellent processing rate - ready for large-scale batch processing"
            )
        elif stats.theorems_per_second > 1.0:
            console.print("  ✅ Good processing rate - suitable for regular batch jobs")
        else:
            console.print(
                "  ⚠️  Consider increasing workers or optimizing for better performance"
            )
        ...
        if stats.failed_files > 0:
            ...
            if failure_rate > 20:
                console.print(
                    f"  ⚠️  High failure rate ({failure_rate:.1f}%) - check file formats and dependencies"
                )
            else:
                console.print(
                    f"  ℹ️  Some files failed ({failure_rate:.1f}%) - see error summary above"
                )
