    ...
    if contexts:
        ...
        for context, data in contexts.items():
            ...
            status = (
                ...
                else "⚠️" if success_rate >= target_rate * 0.5 else "❌"
            )

            click.echo(
                f"  {context.title():<12}: {success_rate:.1%} ({attempts:3d} attempts) "
    ...
    if strategy_rankings:
        ...
        for context, rankings in strategy_rankings.items():
            ...
            for i, rank in enumerate(rankings[:3]):  # Top 3
                ...
                medal = ["🥇", "🥈", "🥉"][i] if i < 3 else "  "
                click.echo(
                    f"    {medal} {strategy:<20}: {success_rate:.1%} ({attempts} attempts)"
                )
            click.echo()
...
def analyze(file_path: Path, show_strategy: bool):
    ...
    try:
        ...
        if show_strategy:
            ...
            target_rates = {"arithmetic": "85%", "mixed": "40%", "complex": "25%"}
        ...
        click.echo(f"  {status}")
        click.echo(
            f"  Theorems found: {len(result.theorems) if result.theorems else 0}"
        )
...
def manage(reset: bool, export: Optional[Path]):
    ...
    if reset:
        if click.confirm(
            "⚠️  This will reset all optimization learning data. Continue?"
        ):
...
def benchmark():
    ...
    click.echo(
        "This will test various strategies across different mathematical contexts."
    )
    click.echo()
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "test_optimization_system.py"],
            capture_output=True,
            text=True,
            cwd=Path.cwd(),
        )
