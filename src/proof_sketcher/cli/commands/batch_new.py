...
@click.option(
    "--ignore", multiple=True, help="Additional patterns to ignore (e.g., '**/test/**')"
)
@click.option(
    "--claude/--offline",
    default=True,
...
@click.option(
    "--report", type=click.Path(path_type=Path), help="Save detailed processing report"
)
@click.option("--dry-run", is_flag=True, help="Analyze project without processing")
@click.option("--clean-cache", is_flag=True, help="Clean cache before processing")
def batch(
    project_dir: Path,
    output: Path,
    ...
    if use_cache:
        cache_manager = CacheManager(
            cache_dir=Path(cache_dir), ttl_days=30, compress=True
        )
        ...
        if clean_cache:
            ...
            console.print(
                f"📦 Cache: {stats['total_entries']} entries ({stats['total_size_mb']:.1f} MB)"
            )
    ...
    stats_table.add_row(
        "Dependency Graph",
        f"{stats['dependency_graph']['nodes']} nodes, {stats['dependency_graph']['edges']} edges",
    )
    stats_table.add_row(
        "Is DAG",
        "✅ Yes" if stats["dependency_graph"]["is_dag"] else "❌ No (has cycles)",
    )
    ...
    processor = ParallelProcessor(
        max_workers=parallel, use_processes=True  # Use processes for better parallelism
    )
    ...
    try:
        ...
        process_results = asyncio.run(
            processor.process_project(
                project_data, output, processing_options, progress_callback
            )
        )
        ...
        console.print(
            f"  • Throughput: {perf_stats['theorems_per_second']:.2f} theorems/second"
        )
        ...
        if process_results["errors"] > 0:
            ...
            for error_type, errors in list(error_summary["by_type"].items())[:3]:
                ...
                if errors:
                    console.print(
                        f"    Example: {errors[0]['theorem']} - {errors[0]['message'][:50]}..."
                    )
        ...
        if process_results["processed"] > 0:
            ...
            console.print("\n💡 Next steps:")
            console.print(
                f"  1. View generated documentation: open {output}/index.html"
            )
