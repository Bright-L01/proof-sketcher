...
from ...core.monitoring import global_health_monitor, get_health_status
...
def _display_status_table():
    ...
    for name, check in health_status["health_checks"].items():
        ...
        click.echo(f"{click.style(status_icon, fg=status_color)} {name}{critical_marker}")
        ...
        if check["last_run"]:
            last_run = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(check["last_run"]))
    ...
    click.echo(f"Process Memory: {system.get('process_memory', 0) / 1024 / 1024:.1f} MB")
    click.echo()
    ...
    if total_cache_requests > 0:
        ...
        click.echo(f"Cache Hit Rate: {click.style(f'{hit_rate:.1f}%', fg=hit_rate_color)}")
...
@click.option(
    "--duration",
    type=float,
    help="How long to monitor (seconds, default: indefinite)"
)
...
@click.option(
    "--metric",
    type=str,
    help="Specific metric to show (default: all)"
)
@click.option(
    "--format",
    "output_format",
        ...
        if metric:
            ...
            if metric in perf_report.get("application", {}).get("metrics", {}):
                ...
                if output_format == "json":
                    ...
                    click.echo(f"Min: {metric_data.get('min', 0):.3f} {metric_data.get('unit', '')}")
                    click.echo(f"Max: {metric_data.get('max', 0):.3f} {metric_data.get('unit', '')}")
                    click.echo(f"Average: {metric_data.get('avg', 0):.3f} {metric_data.get('unit', '')}")
                    click.echo(f"Latest: {metric_data.get('latest', 0):.3f} {metric_data.get('unit', '')}")
            else:
...
def _display_metrics_table(perf_report: dict):
    ...
    if app_metrics:
        ...
        click.echo("-" * 80)
        click.echo(f"{'Metric':<30} {'Count':<8} {'Min':<10} {'Max':<10} {'Avg':<10} {'Latest':<10}")
        click.echo("-" * 80)
    ...
    if system:
        ...
        if memory:
            click.echo(f"Memory Used: {memory.get('used', 0) / 1024 / 1024 / 1024:.1f} GB")
            click.echo(f"Memory Available: {memory.get('available', 0) / 1024 / 1024 / 1024:.1f} GB")
...
@click.option(
    "--clear-metrics",
    is_flag=True,
    help="Clear performance metrics"
)
@click.option(
    "--clear-cache",
    is_flag=True,
...
def optimize():
    ...
    try:
        ...
        click.echo()
        click.echo("Use 'proof-sketcher performance benchmark' to measure improvements.")
