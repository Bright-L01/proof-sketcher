...
class BatchStats:
    ...
    def display(self, console: Console) -> None:
        ...
        table.add_row(
            ...
            f"{
                self.files_per_second:.1f}/sec",
        )
        table.add_row(
            "Successful",
            str(self.successful_files),
        ...
        table.add_row(
            "Theorems Found",
            str(self.total_theorems),
            f"{
                self.theorems_per_second:.1f}/sec",
        )
...
class BatchProcessor:
    ...
    def add_files(self, file_paths: List[Path], priority: int = 0) -> None:
        ...
        self.logger.info(
            f"Added {len(file_paths)} files to batch queue (total: {len(self.jobs)})"
        )
        ...
        try:
            ...
            with Progress(
                ...
                await self._process_files_parallel(
                    self.jobs, output_dir, progress, task
                )
    ...
    async def _process_files_parallel(
        self, jobs: List[BatchJob], output_dir: Path, progress: Progress, task: TaskID
    ) -> None:
        ...
        try:
            ...
            for theorem in parse_result.theorems:
                try:
                    ...
                    result.errors.append(
                        f"System error during generation of {
                            theorem.name}: {e}"
                    )
            ...
            for export_format in self.export_formats:
                try:
                    ...
                    result.errors.append(
                        f"File system error during export to {export_format}: {e}"
                    )
                except MemoryError as e:
                    result.errors.append(
                        f"Out of memory during export to {export_format}: {e}"
                    )
    ...
    def _export_sketches(
        ...
        # Setup exporter
        export_options = ExportOptions.model_validate({"output_dir": format_dir})
        ...
        for sketch in sketches:
            try:
                ...
                self.logger.error(
                    f"File system error exporting {
                        sketch.theorem_name}: {e}"
                )
            except MemoryError as e:
    ...
    def _calculate_stats(self, total_time_ms: float) -> BatchStats:
        ...
        files_per_second = (
            (total_files * 1000) / total_time_ms if total_time_ms > 0 else 0
        )
        theorems_per_second = (
            (total_theorems * 1000) / total_time_ms if total_time_ms > 0 else 0
        )
        avg_time_per_file = total_time_ms / total_files if total_files > 0 else 0
        avg_time_per_theorem = (
            total_time_ms / total_theorems if total_theorems > 0 else 0
        )
