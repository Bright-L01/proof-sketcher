...
class HealthMonitor:
        ...
        def check_lean_executable() -> bool:
            ...
            try:
                ...
                result = subprocess.run(
                    ["lean", "--version"], capture_output=True, timeout=5
                )
    ...
    async def run_all_health_checks(self) -> Dict[str, bool]:
        ...
        tasks = {
            name: self.run_health_check(name) for name in self.health_checks.keys()
        }
    ...
    def check_alert_rules(self) -> None:
        ...
        if (
            context.get("theorems_processed", 0) > 0
            and context.get("errors_encountered", 0) > 0
        ):
            context["error_rate"] = (
                context["errors_encountered"] / context["theorems_processed"]
            ) * 100
        else:
        ...
        if (
            context.get("theorems_processed", 0) > 0
            and context.get("processing_time_total", 0) > 0
        ):
            context["avg_processing_time"] = (
                context["processing_time_total"] / context["theorems_processed"]
            )
        else:
    ...
    async def _monitoring_loop(self, interval: float) -> None:
        ...
        while self._monitoring_active:
            try:
                ...
                global_monitor.record_metric("monitoring_cycle_completed", current_time)
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        # Overall health
        critical_checks = [
            check for check in self.health_checks.values() if check.critical
        ]

        critical_failing = [
            check for check in critical_checks if check.last_result is False
        ]
