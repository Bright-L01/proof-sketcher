...
class TempFileManager:
    ...
    def cleanup_all(self) -> int:
        ...
        # Clean up files
        files_to_remove = list(self.temp_files)  # Copy to avoid modification during iteration
    ...
    def register_process(self, process: psutil.Process) -> None:
        ...
        with self.lock:
            ...
            if len(self.active_processes) >= self.max_processes:
                raise ResourceError(
                    f"Too many active processes ({len(self.active_processes)} >= {self.max_processes})",
...
class ResourceMonitor:
    ...
    def check_limits(self, usage: Optional[ResourceUsage] = None) -> None:
        ...
        if (self.limits.max_memory_mb and 
            ...
            raise MemoryError(
                ...
                context={"current_memory_mb": usage.memory_mb, "limit_mb": self.limits.max_memory_mb}
            )
        
        # Check disk space
        if (self.limits.max_disk_mb and 
            usage.disk_used_mb > self.limits.max_disk_mb):
            raise DiskSpaceError(
                f"Disk usage exceeds limit",
    ...
    def add_warning_callback(self, callback: Callable[[str, ResourceUsage], None]) -> None:
        """Add callback for resource warnings."""
        self.warning_callbacks.append(callback)
    
    def add_error_callback(self, callback: Callable[[str, ResourceUsage], None]) -> None:
    ...
    def _check_warnings(self, usage: ResourceUsage) -> None:
        ...
        if (self.limits.max_memory_mb and 
            usage.memory_mb > self.limits.max_memory_mb * 0.8):
        ...
        if usage.disk_available_mb < 500:
            warnings.append(f"Low disk space: {usage.disk_available_mb:.1f} MB available")
