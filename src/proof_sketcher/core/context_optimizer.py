...
class MathematicalContextDetector:
    ...
    def detect_context(self, theorem_content: str, theorem_name: str = "") -> MathematicalContext:
        ...
        arithmetic_score = sum(1 for pattern in self.arithmetic_patterns 
...
class ContextOptimizer:
    ...
    def select_strategy(self, context: MathematicalContext) -> OptimizationStrategy:
        ...
        total_trials = sum(arm.performance.successes + arm.performance.failures 
                          for arm in arms)
        ...
        self.logger.debug(f"Selected {best_arm.strategy.value} for {context.value} "
                         f"(UCB1: {best_arm.calculate_ucb1_score(total_trials):.3f})")
        
        return best_arm.strategy
    
    def update_performance(self, context: MathematicalContext, 
                          strategy: OptimizationStrategy,
        ...
        if len(self.recent_results) >= 20:  # Sufficient data
            recent_success_rate = sum(1 for r in list(self.recent_results)[-20:] if r['success']) / 20
        ...
        if self.overall_performance.successes + self.overall_performance.failures % 50 == 0:
    ...
    def get_optimization_stats(self) -> Dict:
        ...
        """
        total_attempts = self.overall_performance.successes + self.overall_performance.failures
        ...
        for context, arms in self.bandits.items():
            context_total = sum(arm.performance.successes + arm.performance.failures for arm in arms)
            context_successes = sum(arm.performance.successes for arm in arms)
            context_rate = context_successes / context_total if context_total > 0 else 0.0
            
            best_arm = max(arms, key=lambda a: a.performance.success_rate) if arms else None
            ...
            context_stats[context.value] = {
                ...
                'distribution_pct': self.context_distribution[context] / total_attempts * 100 if total_attempts > 0 else 0
            }
        ...
        return {
            'overall': {
                ...
                'progress_to_target': current_rate / target_rate if target_rate > 0 else 0,
    ...
    def optimize_for_theorem(self, theorem_content: str, theorem_name: str = "") -> Tuple[MathematicalContext, OptimizationStrategy]:
        ...
        self.logger.info(f"Optimized theorem '{theorem_name}': context={context.value}, strategy={strategy.value}")
    ...
    def _get_strategy_rankings(self) -> Dict:
        ...
        rankings = {}
        for context, arms in self.bandits.items():
            sorted_arms = sorted(arms, key=lambda a: a.performance.success_rate, reverse=True)
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        for context, target_rate in self.target_rates.items():
            if context in self.bandits:
                arms = self.bandits[context]
                total_attempts = sum(arm.performance.successes + arm.performance.failures for arm in arms)
                total_successes = sum(arm.performance.successes for arm in arms)
                current_rate = total_successes / total_attempts if total_attempts > 0 else 0.0
                
                if total_attempts >= 10:  # Sufficient data
                    if current_rate < target_rate * 0.8:
                        recommendations.append(f"Underperforming in {context.value}: {current_rate:.1%} vs target {target_rate:.1%}")
                    elif current_rate > target_rate * 1.2:
                        recommendations.append(f"Exceeding target in {context.value}: consider harder test cases")
        ...
        if overall_rate >= 0.45:  # Close to 50% target
            recommendations.append("🎯 Approaching 50% target! Consider fine-tuning strategies")
        elif overall_rate >= 0.35:
            recommendations.append("📈 Good progress toward 50% target")
        elif overall_rate < 0.25:
            recommendations.append("⚠️ Below expected baseline - review strategy selection")
    ...
    def _load_performance_data(self) -> None:
        ...
        try:
                        ...
                        if context in self.bandits:
                            for i, arm_data in enumerate(arms_data):
                                if i < len(self.bandits[context]):
                                    ...
                                    arm.performance.successes = arm_data.get('successes', 0)
                                    arm.performance.failures = arm_data.get('failures', 0)
                                    arm.performance.total_time = arm_data.get('total_time', 0.0)
                                    arm.performance.last_updated = arm_data.get('last_updated', time.time())
                    except ValueError:
            ...
            if 'overall_performance' in data:
                ...
                self.overall_performance.last_updated = perf_data.get('last_updated', time.time())
