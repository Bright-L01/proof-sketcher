...
class FixedAIGenerator(IGenerator):
    ...
    def __init__(
        ...
        if ai_client:
            ...
            if anthropic_client.is_available():
                ...
                raise GeneratorError("No AI service available and offline fallback disabled")
    ...
    def generate_eli5_explanation(
        ...
        prompt = f"""Explain this theorem like I'm five years old:
...
Make it simple and use analogies a child would understand."""
    ...
    def generate_tactic_explanation(
        ...
        prompt = f"""Explain the Lean tactics used in this proof:
...
Focus on why each tactic is used and what it accomplishes."""
    ...
    def generate_step_by_step(
        ...
        prompt = f"""Provide a detailed step-by-step walkthrough of this proof:
...
Break down each step and explain the reasoning."""
    ...
    def generate_streaming(
        ...
        if generation_type == GenerationType.PROOF_SKETCH:
            result = str(self.generate_proof_sketch(theorem, config, mathematical_context))
        elif generation_type == GenerationType.ELI5:
            result = self.generate_eli5_explanation(theorem, config, mathematical_context)
        elif generation_type == GenerationType.TACTIC_EXPLANATION:
            result = self.generate_tactic_explanation(theorem, config, mathematical_context)
    ...
    def _build_proof_sketch_prompt(
        ...
        prompt = f"""Generate a natural language proof sketch for this Lean theorem.
    ...
    def _parse_proof_sketch(self, response: str, theorem: TheoremInfo) -> ProofSketch:
        ...
        try:
            ...
            return ProofSketch(
                ...
                key_steps=[
                    ProofStep(**step) for step in data.get("key_steps", [])
                ],
