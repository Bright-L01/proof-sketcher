...
class MarkdownExporter(BaseExporterImpl):
    ...
    def __init__(
        ...
        self._link_mode = getattr(options, 'link_mode', 'relative')  # 'relative' or 'absolute'
        self._generate_toc = getattr(options, 'generate_toc', True)
        self._use_collapsible = getattr(options, 'use_collapsible', True)
        self._math_notation = getattr(options, 'math_notation', 'dollars')  # 'dollars' or 'latex'
    ...
    def _create_template_context_github(
        ...
        if self._use_collapsible:
            base_context["collapsible_sections"] = self._create_collapsible_sections(sketch)
    ...
    def _process_animation_for_github(self, animation_path: Path, theorem_name: str) -> Dict:
    ...
    def _generate_enhanced_toc(
        ...
        toc_content.extend([
            ...
            "- [🎥 Animated Proofs](#-animated-proofs)" if context.animations else "",
        ...
        if context.animations:
            animated_theorems = [s for s in sketches if s.theorem_name in context.animations]
            toc_content.extend([
                "## 🎥 Animated Proofs",
                "",
        ...
        for letter in sorted(alpha_groups.keys()):
            ...
            if self._use_collapsible:
                ...
                toc_content.extend([
                    f"### {letter} ({len(theorems)} theorems)",
                    ""
                ])
        ...
        toc_content.extend(["", "---", "", f"*Generated on {context.timestamp.isoformat() if context.timestamp else 'unknown date'}*"])
    ...
    def _group_by_difficulty(self, sketches: List[ProofSketch]) -> Dict[str, List[ProofSketch]]:
    ...
    def _generate_animation_previews(
        ...
        for sketch in sketches:
            if sketch.theorem_name in context.animations:
                ...
                content.extend([
                    f"## [{sketch.theorem_name}]({theorem_url})",
                    ""
                ])
