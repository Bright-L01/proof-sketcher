...
async def demonstrate_performance_improvements():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(f"Average time per file: {baseline_results['total_time'] / baseline_results['files_processed']:.3f}s")
        ...
        print(f"Average time per file: {optimized_results['total_time'] / optimized_results['files_processed']:.3f}s")
        ...
        memory_improvement = ((baseline_profile.memory_growth - optimized_profile.memory_growth) 
                             / max(baseline_profile.memory_growth, 1)) * 100
        ...
        if metrics_summary.get('metrics'):
            ...
            for name, data in list(metrics_summary['metrics'].items())[:3]:
                print(f"  {name}: {data.get('avg', 0):.3f} {data.get('unit', '')} (avg)")
        ...
        print(f"✨ Monitoring metrics: {len(metrics_summary.get('metrics', {})) + len(metrics_summary.get('counters', {}))}")
