...
def simulate_theorem_processing(count: int, use_optimizations: bool = False) -> dict:
    ...
    return {
        ...
        "throughput": processed / (end_time - start_time) if end_time > start_time else 0
    }
...
def main():
    ...
    with profiler.profile("baseline") as baseline_profile:
        baseline_results = simulate_theorem_processing(theorem_count, use_optimizations=False)
    ...
    with profiler.profile("optimized") as optimized_profile:
        ...
        optimized_results = simulate_theorem_processing(theorem_count, use_optimizations=True)
    ...
    time_improvement = ((baseline_results['total_time'] - optimized_results['total_time']) 
                       / baseline_results['total_time']) * 100
    
    throughput_improvement = ((optimized_results['throughput'] - baseline_results['throughput']) 
                             / baseline_results['throughput']) * 100
    
    cache_hit_rate = (optimized_results['cache_hits'] / optimized_results['theorems_processed']) * 100
    ...
    global_monitor.record_metric("processing_throughput", optimized_results['throughput'], "theorems/sec")
    ...
    print(f"📊 {len(metrics_summary.get('metrics', {})) + len(metrics_summary.get('counters', {}))} metrics tracked")
    print()
