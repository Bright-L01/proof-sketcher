...
async def run_batch_processing_demo():
    ...
    print(
        f"   • Is DAG: {'✅ Yes' if stats['dependency_graph']['is_dag'] else '❌ No'}"
    )
    ...
    cache_manager = CacheManager(
        cache_dir=cache_dir, ttl_days=30, compress=True, max_cache_size_mb=100
    )
    ...
    results = await processor.process_project(
        project_data, output_dir, options, progress_callback
    )
    ...
    print(
        f"   • Successfully processed: {results['processed']} ({results['processed']/results['total_theorems']*100:.1f}%)"
    )
    ...
    print(
        f"   • Entries added: {final_cache_stats['total_entries'] - cache_stats['total_entries']}"
    )
    ...
    if results["processed"] > 0:
        ...
        second_results = await processor.process_project(
            project_data, output_dir, options, lambda *args: None  # Silent progress
        )
