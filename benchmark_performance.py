...
class PerformanceBenchmark:
    ...
    async def _benchmark_batch_processing(
        ...
        try:
            ...
            return {
                ...
                "throughput": stats.total_theorems / (end_time - start_time) if end_time > start_time else 0,
                "cache_stats": self._get_cache_stats() if version == "optimized" else None
            }
    ...
    def _generate_comparison_report(self):
        ...
        report.append("")
        report.append("| Scale | Baseline Time (s) | Optimized Time (s) | Improvement | Baseline Throughput | Optimized Throughput | Memory Improvement |")
        report.append("|-------|-------------------|-------------------|-------------|---------------------|----------------------|-------------------|")
        
        for scale in scales:
            baseline = next((r for r in self.results["baseline"] if r["scale"] == scale), None)
            optimized = next((r for r in self.results["optimized"] if r["scale"] == scale), None)
            
            if baseline and optimized and "error" not in baseline and "error" not in optimized:
                time_improvement = ((baseline["duration"] - optimized["duration"]) / baseline["duration"]) * 100
                throughput_improvement = ((optimized["throughput"] - baseline["throughput"]) / baseline["throughput"]) * 100 if baseline["throughput"] > 0 else 0
                memory_improvement = ((baseline["memory_usage"] - optimized["memory_usage"]) / baseline["memory_usage"]) * 100 if baseline["memory_usage"] > 0 else 0
                
                report.append(f"| {scale} | {baseline['duration']:.2f} | {optimized['duration']:.2f} | {time_improvement:+.1f}% | {baseline['throughput']:.2f} | {optimized['throughput']:.2f} | {memory_improvement:+.1f}% |")
        ...
        total_baseline_time = sum(r["duration"] for r in self.results["baseline"] if "error" not in r)
        total_optimized_time = sum(r["duration"] for r in self.results["optimized"] if "error" not in r)
        
        if total_baseline_time > 0:
            overall_improvement = ((total_baseline_time - total_optimized_time) / total_baseline_time) * 100
            report.append(f"- **Overall Processing Time**: {overall_improvement:.1f}% improvement")
        
        avg_baseline_throughput = np.mean([r["throughput"] for r in self.results["baseline"] if "error" not in r and r["throughput"] > 0])
        avg_optimized_throughput = np.mean([r["throughput"] for r in self.results["optimized"] if "error" not in r and r["throughput"] > 0])
        
        if avg_baseline_throughput > 0:
            throughput_improvement = ((avg_optimized_throughput - avg_baseline_throughput) / avg_baseline_throughput) * 100
            report.append(f"- **Average Throughput**: {throughput_improvement:.1f}% improvement")
    ...
    def _generate_performance_charts(self):
        ...
        try:
            ...
            baseline_scales = [r["scale"] for r in self.results["baseline"] if "error" not in r]
            baseline_times = [r["duration"] for r in self.results["baseline"] if "error" not in r]
            baseline_throughput = [r["throughput"] for r in self.results["baseline"] if "error" not in r]
            
            optimized_scales = [r["scale"] for r in self.results["optimized"] if "error" not in r]
            optimized_times = [r["duration"] for r in self.results["optimized"] if "error" not in r]
            optimized_throughput = [r["throughput"] for r in self.results["optimized"] if "error" not in r]
            ...
            ax1.plot(baseline_scales, baseline_times, 'o-', label='Baseline', color='red')
            ax1.plot(optimized_scales, optimized_times, 'o-', label='Optimized', color='green')
            ...
            ax2.plot(baseline_scales, baseline_throughput, 'o-', label='Baseline', color='red')
            ax2.plot(optimized_scales, optimized_throughput, 'o-', label='Optimized', color='green')
            ...
            bars = ax3.bar(scales_with_improvements, improvements, color='skyblue', alpha=0.7)
            ...
            baseline_memory = [r["memory_usage"] / 1024 / 1024 for r in self.results["baseline"] if "error" not in r]  # Convert to MB
            optimized_memory = [r["memory_usage"] / 1024 / 1024 for r in self.results["optimized"] if "error" not in r]
            
            ax4.plot(baseline_scales, baseline_memory, 'o-', label='Baseline', color='red')
            ax4.plot(optimized_scales, optimized_memory, 'o-', label='Optimized', color='green')
    ...
    def _generate_summary_report(self):
        ...
        summary = {
            ...
            "total_tests_run": len(self.results["baseline"]) + len(self.results["optimized"]),
    ...
    def _calculate_key_metrics(self) -> Dict[str, Any]:
        ...
        for scale in set(r["scale"] for r in baseline_valid):
            ...
            if baseline and optimized:
                if baseline["duration"] > 0:
                    time_imp = ((baseline["duration"] - optimized["duration"]) / baseline["duration"]) * 100
                    avg_time_improvement += time_imp
                
                if baseline["throughput"] > 0:
                    throughput_imp = ((optimized["throughput"] - baseline["throughput"]) / baseline["throughput"]) * 100
                    avg_throughput_improvement += throughput_imp
                
                if baseline["memory_usage"] > 0:
                    memory_imp = ((baseline["memory_usage"] - optimized["memory_usage"]) / baseline["memory_usage"]) * 100
        ...
        return {
            ...
            "average_throughput_improvement_percent": round(avg_throughput_improvement, 2),
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if isinstance(key_metrics, dict) and "error" not in key_metrics:
            if key_metrics.get("average_memory_improvement_percent", 0) < 10:
                recommendations.append("Consider tuning cache size limits for better memory efficiency")
            
            if key_metrics.get("average_throughput_improvement_percent", 0) > 50:
                recommendations.append("Significant throughput improvements observed - optimizations are highly effective")
