...
from proof_sketcher.parser.models import TheoremInfo, ParseResult, ParseError
    ...
    def test_parse_directory(self, tmp_path, parser_config):
        ...
        with patch.object(parser, 'parse_file') as mock_parse:
            mock_parse.return_value = ParseResult(
                theorems=[TheoremInfo(name="test", statement="test")],
                success=True
            )
...
"""
        ...
        with patch.object(parser.extractor, 'extract_theorems') as mock_extract:
            ...
            mock_extract.return_value = [
                ...
                TheoremInfo(name="another_valid", statement="False → True", proof="by intro h; trivial")
            ]
...
class TestEnhancedLeanParser:
    ...
    def test_semantic_analysis(self, complex_lean_content, parser_config):
        ...
        assert semantics["abstraction_level"] in ["concrete", "abstract", "very_abstract"]
