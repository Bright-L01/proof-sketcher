...
def test_semantic_generator_levels():
    ...
    try:
        from proof_sketcher.generator import EducationalLevel, SemanticGenerator
...
def test_progressive_explanations():
    ...
    try:
        from proof_sketcher.generator import SemanticGenerator
        from proof_sketcher.parser import SimpleLeanParser
...
def test_mathematical_entity_recognition():
    ...
    try:
        ...
        test_cases = [
            (
                "Number theory",
                "theorem nt (n : Nat) (h : n > 0) : n + 1 > 1 := by simp [h]",
            ),
...
def test_tactic_aware_explanations():
    ...
    try:
        ...
        test_cases = [
            ...
            (
                "Rewrite proof",
                "theorem rw_proof (a b : Nat) : a + b = b + a := by rw [Nat.add_comm]",
            ),
            (
                "Induction proof",
                """
        ...
        for name, content in test_cases:
            ...
            try:
                ...
                if result.success and result.theorems:
                    ...
                    print(
                        f"  ⚙️ Tactics found: {[t.name for t in theorem.tactic_sequence]}"
                    )
                    ...
                    full_text = (
                        sketch.introduction
                        + " "
                        + " ".join(step.description for step in sketch.key_steps)
                    ).lower()

                    if "simp" in content:
                        has_simp_explanation = any(
                            word in full_text
                            for word in ["simplif", "automatic", "rules"]
                        )
                        print(f"  ✅ Explains simplification: {has_simp_explanation}")
                    elif "rw" in content:
                        has_rw_explanation = any(
                            word in full_text
                            for word in ["rewrite", "replac", "substitut"]
                        )
                        print(f"  ✅ Explains rewriting: {has_rw_explanation}")
                    elif "induction" in content:
                        has_ind_explanation = any(
                            word in full_text
                            for word in ["induction", "inductive", "base case"]
                        )
...
def test_backward_compatibility():
    ...
    try:
        ...
        for i, sketch in enumerate([sketch1, sketch2, sketch3], 1):
            if sketch:
                print(
                    f"  ✅ Sketch {i}: {sketch.theorem_name}, {len(sketch.key_steps)} steps"
                )
            else:
