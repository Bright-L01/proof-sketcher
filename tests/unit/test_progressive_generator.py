...
def test_progressive_explanation_generation():
    ...
    try:
        from proof_sketcher.generator import EducationalLevel, ProgressiveGenerator
        ...
        try:
            ...
            print(
                f"✅ Generated progressive explanation for: {progressive.theorem_name}"
            )
            ...
            print(
                f"🔍 Exploration suggestions: {len(progressive.exploration_suggestions)}"
            )
            ...
            for level in expected_levels:
                if level in progressive.levels:
                    ...
                    print(
                        f"  ✅ {level.capitalize()} level: {len(sketch.key_steps)} steps"
                    )
...
def test_learning_pathway_creation():
    ...
    try:
        from proof_sketcher.generator import ProgressiveGenerator
        from proof_sketcher.parser import SimpleLeanParser
        ...
        try:
            ...
            if result.success and result.theorems:
                ...
                print(
                    f"📋 Learning pathway has {len(progressive.learning_pathway)} steps:"
                )
                ...
                has_progression = (
                    "novice" in levels
                    and "intermediate" in levels
                    and "advanced" in levels
                )
    ...
    try:
        ...
        try:
            ...
            if result.success and result.theorems:
                ...
                has_natural_numbers = any(
                    "natural" in name or "nat" in name for name in concept_names
                )
...
def test_comprehensive_integration():
    ...
    try:
        from proof_sketcher.exporter import HTMLBaseExporter
        from proof_sketcher.generator import ProgressiveGenerator
        from proof_sketcher.parser import SimpleLeanParser
        ...
        try:
            ...
            if result.success and result.theorems:
                ...
                checklist = {
                    ...
                    "Maturity requirements": len(
                        progressive.mathematical_maturity_required
                    )
                    > 0,
                    "Interactive elements": len(progressive.exploration_suggestions)
                    > 0,
