...
""")
        ...
        # Test parsing
        parse_result = runner.invoke(cli, [
            'parse', str(lean_file),
            '--output', str(temp_output_dir / 'parsed.json')
        ])
        ...
        with patch('proof_sketcher.generator.claude.ClaudeClient.generate') as mock_generate:
            ...
            generate_result = runner.invoke(cli, [
                'generate', str(parsed_file),
                '--output', str(temp_output_dir / 'proof_sketch.json')
            ])
            assert generate_result.exit_code == 0
        
        # Test export to HTML
        export_result = runner.invoke(cli, [
            'export', str(temp_output_dir / 'proof_sketch.json'),
            '--format', 'html',
            '--output', str(temp_output_dir / 'output.html')
        ])
...
"""
        ...
        with patch('proof_sketcher.generator.claude.ClaudeClient.generate') as mock_generate:
            ...
            result = runner.invoke(cli, [
                'prove', str(lean_file),
                '--format', 'all',
                '--output', str(temp_output_dir / 'mathlib_output')
            ])
            assert result.exit_code == 0
...
class TestPerformanceBenchmarks:
    ...
    def test_single_theorem_performance(self, benchmark, test_data_dir, temp_output_dir):
        ...
        def process_theorem():
            result = runner.invoke(cli, [
                'prove', str(lean_file),
                '--offline',
                '--output', str(temp_output_dir / 'perf')
            ])
            assert result.exit_code == 0
    ...
    def test_batch_performance(self, benchmark, test_data_dir, temp_output_dir):
        ...
        def process_batch():
            result = runner.invoke(cli, [
                ...
                '--parallel', '4',
                '--offline',
                '--output', str(temp_output_dir / 'batch_perf')
            ])
            assert result.exit_code == 0
    ...
    def test_cache_effectiveness(self, test_data_dir, temp_output_dir):
        ...
        result1 = runner.invoke(cli, [
            'prove', str(lean_file),
            '--offline',
            '--output', str(temp_output_dir / 'cache1')
        ])
        ...
        result2 = runner.invoke(cli, [
            'prove', str(lean_file),
            '--offline',
            '--output', str(temp_output_dir / 'cache2')
        ])
