...
"""
    ...
    def mock_proof_sketch(self):
        ...
        return ProofSketch(
            ...
            key_insights=["Induction on the first argument", "Use of definitional equalities"],
            mathematical_context="Natural number arithmetic"
        )
    
    def test_parse_generate_export_pipeline(self, sample_lean_content, temp_project_dir, mock_proof_sketch):
        ...
        with patch('proof_sketcher.parser.lean_parser.LeanParser.parse_file') as mock_parse:
            ...
            # Mock the generator
            with patch('proof_sketcher.generator.ai_generator.AIGenerator.generate_proof_sketch') as mock_generate:
                ...
                with patch('proof_sketcher.exporter.html.HTMLExporter.export_single') as mock_export:
                    mock_export.return_value = ExportResult(
                        ...
                        output_files=[temp_project_dir / "output" / "nat_add_comm.html"],
                    ...
                    # Export
                    exporter = HTMLExporter(ExportOptions(output_dir=temp_project_dir / "output"))
    ...
    async def test_async_animation_pipeline(self, mock_proof_sketch, temp_project_dir):
        ...
        mock_animation_response.video_path = temp_project_dir / "animations" / "nat_add_comm.mp4"
        mock_animation_response.static_path = temp_project_dir / "animations" / "nat_add_comm.png"
        
        with patch('proof_sketcher.animator.animation_generator.TheoremAnimator.animate_proof') as mock_animate:
...
""")
        ...
        with patch('proof_sketcher.batch.parallel_processor.ParallelProcessor') as MockProcessor:
    ...
    def test_error_handling_pipeline(self, temp_project_dir):
        ...
        with patch('proof_sketcher.parser.lean_parser.LeanParser.parse_file') as mock_parse:
            mock_parse.side_effect = ProofSketcherError("Syntax error", error_code="PARSE_001")
...
class TestCLIIntegration:
    ...
    def test_cli_prove_command(self, tmp_path):
        ...
        with patch('proof_sketcher.parser.lean_parser.LeanParser.parse_file') as mock_parse:
            ...
            with patch('proof_sketcher.generator.offline.OfflineGenerator.generate_proof_sketch') as mock_generate:
...
class TestResourceManagement:
    ...
    def test_memory_limits_pipeline(self):
        ...
        from proof_sketcher.core.resources import ResourceMonitor, ResourceLimits
...
class TestPerformanceIntegration:
    ...
    def test_parallel_processing_performance(self, tmp_path):
        ...
        with patch.object(ParallelProcessor, 'process_theorem', side_effect=mock_process):
    ...
    def test_notation_optimization_performance(self):
        ...
        from proof_sketcher.parser.mathlib_notation_optimized import OptimizedMathlibNotationHandler
