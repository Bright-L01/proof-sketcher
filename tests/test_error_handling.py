...
from src.proof_sketcher.exporter.models import ExportOptions, ExportContext
from src.proof_sketcher.core.exceptions import ParserError, GeneratorError, ExportError
...
class TestGeneratorErrorHandling:
    ...
    def test_ai_generator_with_invalid_client(self):
        ...
        generator = FixedAIGenerator(ai_client=failing_client, use_offline_fallback=True)
    ...
    def test_anthropic_client_network_error(self, monkeypatch):
        ...
        with patch('src.proof_sketcher.ai.anthropic_client.Anthropic') as mock_anthropic:
            mock_client = Mock()
...
class TestSystemErrorHandling:
    ...
    def test_resource_cleanup(self, temp_dir):
        ...
        for i in range(5):
            with tempfile.NamedTemporaryFile(mode='w', suffix='.lean', dir=temp_dir, delete=False) as f:
