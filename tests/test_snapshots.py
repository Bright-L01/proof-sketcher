...
class SnapshotManager:
    ...
    def compare_snapshots(self, actual: str, expected: str, 
                         ignore_patterns: Optional[list] = None) -> bool:
...
def complex_proof_sketch():
    ...
    return ProofSketch(
        ...
        key_insights=["Inverse relationship", "Continuity requirement", "Geometric interpretation"],
...
class TestHTMLSnapshots:
    ...
    def test_complex_notation_html(self, complex_proof_sketch, snapshot_manager, tmp_path):
        ...
        if expected is None:
            snapshot_manager.save_snapshot("complex_notation_html", html_content, "html")
...
class TestMarkdownSnapshots:
    ...
    def test_github_markdown_export(self, sample_proof_sketch, snapshot_manager, tmp_path):
        """Test GitHub-flavored Markdown export."""
        exporter = MarkdownExporter(ExportOptions(
            output_dir=tmp_path,
...
class TestLaTeXSnapshots:
    ...
    def test_latex_math_environments(self, complex_proof_sketch, snapshot_manager, tmp_path):
...
class TestNotationSnapshots:
    ...
    def test_notation_conversions(self, snapshot_manager):
        ...
        snapshot_path = snapshot_manager.get_snapshot_path("notation_conversions", "json")
...
class TestSnapshotValidation:
    ...
    def test_snapshot_html_validity(self, snapshot_manager):
        ...
        for snapshot_path in html_snapshots:
            ...
            assert content.strip().startswith("<!DOCTYPE html>") or content.strip().startswith("<")
            assert "</html>" in content or snapshot_path.name.endswith("_fragment.html")
            
            # Check for common issues
            assert content.count("<div") == content.count("</div"), f"Unclosed divs in {snapshot_path.name}"
            assert content.count("<span") == content.count("</span"), f"Unclosed spans in {snapshot_path.name}"
    ...
    def test_snapshot_markdown_validity(self, snapshot_manager):
        ...
        for snapshot_path in md_snapshots:
            ...
            assert code_block_count % 2 == 0, f"Unclosed code blocks in {snapshot_path.name}"
