...
from src.proof_sketcher.core.batch_processor import (
    BatchProcessor, BatchJob, BatchResult, BatchStats
)
...
from src.proof_sketcher.parser.models import ParseResult, TheoremInfo, FileMetadata
...
class TestBatchProcessor:
    ...
    async def test_process_single_file_success(self, batch_processor, temp_dir):
        ...
        with patch.object(batch_processor.parser, 'parse_file', return_value=mock_parse_result):
            with patch.object(batch_processor.generator, 'generate_proof_sketch') as mock_gen:
    ...
    def test_calculate_stats(self, batch_processor):
        ...
        with patch.object(batch_processor.resource_monitor, 'get_current_usage') as mock_usage:
    ...
    async def test_parallel_processing(self, batch_processor, temp_dir):
        ...
        for i, file in enumerate(files):
            mock_parse_results.append(ParseResult(
                theorems=[
                    TheoremInfo(
                        name=f"test{i}",
        ...
        with patch.object(batch_processor.parser, 'parse_file', return_value=mock_parse_result):
            with patch.object(batch_processor.generator, 'generate_proof_sketch') as mock_gen:
                ...
                with patch.object(batch_processor.resource_monitor, 'get_current_usage') as mock_usage:
                    ...
                    with patch.object(batch_processor.logger, 'warning') as mock_warning:
                        stats = await batch_processor.process_batch(output_dir)
...
theorem add_comm (a b : Nat) : a + b = b + a := by
  ...
  | succ n ih => simp [add_succ, ih]
            ...
            with patch.object(processor.parser, 'parse_file') as mock_parse:
                ...
                with patch.object(processor.generator, 'generate_proof_sketch') as mock_gen:
                    from src.proof_sketcher.generator.models import ProofSketch, ProofStep
                    ...
                    # Mock exporters
                    with patch('src.proof_sketcher.exporter.html.HTMLExporter') as MockHTML:
                        with patch('src.proof_sketcher.exporter.markdown.MarkdownExporter') as MockMD:
                            mock_html = MockHTML.return_value
                            mock_md = MockMD.return_value
                            
                            mock_html.export_single.return_value = MagicMock(success=True)
