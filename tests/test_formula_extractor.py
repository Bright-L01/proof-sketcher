...
class TestLeanToLatexConverter:
    ...
    def test_error_handling(self, converter):
        ...
        with patch.object(
            converter, "_convert_unicode_symbols", side_effect=Exception("Test error")
        ):
...
class TestProofStepAnalyzer:
    ...
    def test_identify_transformation_type(self, analyzer):
        ...
        assert (
            analyzer._identify_transformation_type("simp")
            == TransformationType.SIMPLIFICATION
        )
        assert (
            analyzer._identify_transformation_type("norm_num")
            == TransformationType.SIMPLIFICATION
        )
        assert (
            analyzer._identify_transformation_type("ring")
            == TransformationType.SIMPLIFICATION
        )
        assert (
            analyzer._identify_transformation_type("abel")
            == TransformationType.SIMPLIFICATION
        )

        # Rewriting patterns
        assert (
            analyzer._identify_transformation_type("rw [h]")
            == TransformationType.REWRITE
        )
        assert (
            analyzer._identify_transformation_type("rewrite h")
            == TransformationType.REWRITE
        )

        # Substitution patterns
        assert (
            analyzer._identify_transformation_type("subst h")
            == TransformationType.SUBSTITUTION
        )
        assert (
            analyzer._identify_transformation_type("apply h")
            == TransformationType.SUBSTITUTION
        )

        # Induction
        assert (
            analyzer._identify_transformation_type("induction n")
            == TransformationType.INDUCTION_STEP
        )

        # Case analysis
        assert (
            analyzer._identify_transformation_type("cases h")
            == TransformationType.CASE_SPLIT
        )
        assert (
            analyzer._identify_transformation_type("by_cases h")
            == TransformationType.CASE_SPLIT
        )

        # Default
        assert (
            analyzer._identify_transformation_type("unknown")
            == TransformationType.SIMPLIFICATION
        )
    ...
    def test_generate_intermediate_steps(self, analyzer):
        ...
        steps = analyzer._generate_intermediate_steps(
            "a + b", "b + a", TransformationType.REWRITE
        )
...
class TestFormulaExtractor:
    ...
    def test_analyze_proof_transformation(self, extractor):
        ...
        transformation = extractor.analyze_proof_transformation("a + 0", "a", "simp")
        assert isinstance(transformation, FormulaTransformation)
        assert transformation.transformation_type == TransformationType.SIMPLIFICATION
    ...
    def test_extract_key_formulas_from_theorem(self, extractor):
        ...
        theorem = "∀ n : ℕ, n + 0 = n"
        proof = (
            "By induction on n. Base case: 0 + 0 = 0. Inductive step: (n+1) + 0 = n+1."
        )
    ...
    def test_process_proof_content_preserves_order(self, extractor):
        ...
        assert (
            processed.formulas[0].position
            < processed.formulas[1].position
            < processed.formulas[2].position
        )
...
class TestExtractedFormula:
    ...
    def test_extracted_formula_creation(self):
        ...
        formula = ExtractedFormula(
            latex="x^2 + y^2 = z^2", display_mode=True, position=42
        )
...
class TestProcessedContent:
    ...
    def test_processed_content_creation(self):
        ...
        content = ProcessedContent(formulas=formulas, lean_code=lean_code, text=text)
