...
from src.proof_sketcher.animator.fallback import (
    FallbackAnimator,
    StaticAnimationGenerator,
)
...
from src.proof_sketcher.core.exceptions import AnimationTimeoutError, AnimatorError
...
class TestProductionAnimatorComplete:
    ...
    def animator(self):
        ...
        config = AnimationConfig(max_memory_mb=512, max_processing_time=300.0)
        manim_config = ManimConfig(
            output_dir=Path("/tmp/test_animations"), temp_dir=Path("/tmp/test_temp")
        )
        return ProductionAnimator(
            animation_config=config, manim_config=manim_config, use_mock=True
        )
    ...
    async def test_generate_preview_frames(self, animator, sample_proof_sketch):
        ...
        frames = await animator.generate_preview_frames(
            sample_proof_sketch, num_frames=3
        )
    ...
    def test_validate_proof_sketch_errors(self, animator):
        ...
        with pytest.raises(AnimatorError, match="theorem name"):
            animator._validate_proof_sketch(
                ProofSketch(
                    theorem_name="",
                    introduction="Test",
                    key_steps=[
                        ProofStep(
                            step_number=1, description="Step", mathematical_content=""
                        )
                    ],
                    conclusion="Done",
                )
    ...
    async def test_create_latex_preview(self, animator):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            result = await animator._create_latex_preview(
                "test_theorem", "a^2 + b^2 = c^2", animator.animation_config
            )
...
class TestStaticAnimationGeneratorComplete:
    ...
    def test_render_text_to_image_with_pillow(self, generator):
        ...
        result = generator._render_text_to_image("Test text", "test_image.png")
    ...
    def test_render_latex_matplotlib_error(self, generator):
        ...
        with patch(
            "matplotlib.pyplot.figure", side_effect=ImportError("No matplotlib")
        ):
            result = generator.render_latex_to_image(
                r"\int_0^1 x^2 dx", "test_latex.png"
            )
    ...
    async def test_generate_animation_complete(self, generator):
        ...
        response = await generator.generate_static_animation(
            proof_sketch, output_filename="static_test.mp4"
        )
...
class TestFallbackAnimatorComplete:
    ...
    def fallback_animator(self, tmp_path):
        ...
        return FallbackAnimator(
            mcp_client=mcp_client, static_generator=static_generator
        )
    ...
    async def test_animate_with_mcp_success(self, fallback_animator):
        ...
        proof_sketch = ProofSketch(
            ...
            introduction="Test",
            key_steps=[
                ProofStep(step_number=1, description="Step 1", mathematical_content="")
            ],
            conclusion="Done",
        )
        ...
        mock_response = AnimationResponse(
            video_path=Path("/tmp/test.mp4"), duration=30.0
        )

        with patch.object(
            fallback_animator.mcp_client, "render_animation", return_value=mock_response
        ):
...
class TestMemoryLeakDetection:
    ...
    async def test_repeated_animations_no_leak(self):
        ...
        proof_sketch = ProofSketch(
            ...
            key_steps=[
                ProofStep(
                    step_number=i, description=f"Step {i}", mathematical_content=""
                )
                for i in range(1, 6)
            ],
            conclusion="Done",
...
class TestStressTest:
    ...
    async def test_concurrent_animations(self):
        ...
        tasks = [animator.animate_proof(proof) for proof in proofs]
