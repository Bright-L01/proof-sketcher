...
class StressTester:
    ...
    async def test_memory_limits(self):
        ...
        try:
            ...
            self.report_result("memory_limits", "basic_enforcement", "FAILED", 
                             f"No exception raised after allocating {len(data)*5}MB")
                             
        except ResourceLimitExceeded as e:
            self.report_result("memory_limits", "basic_enforcement", "PASSED", 
                             f"Correctly caught memory limit: {e}")
        except Exception as e:
            self.report_result("memory_limits", "basic_enforcement", "ERROR", 
        ...
        try:
            ...
            self.report_result("memory_limits", "async_monitoring", "FAILED", 
                             f"No exception after allocating {result*10}MB")
        except ResourceLimitExceeded as e:
            self.report_result("memory_limits", "async_monitoring", "PASSED", 
                             f"Correctly caught async memory limit: {e}")
        except Exception as e:
            self.report_result("memory_limits", "async_monitoring", "ERROR", 
                             f"Unexpected error: {type(e).__name__}: {e}")

        # Test 3: Check if psutil is available
        try:
            import psutil
            self.report_result("memory_limits", "psutil_available", "PASSED", 
                             f"psutil version {psutil.version_info}")
        except ImportError:
            self.report_result("memory_limits", "psutil_available", "WARNING", 
    ...
    async def test_rate_limits(self):
        ...
        try:
            ...
            self.report_result("rate_limits", "basic_enforcement", "FAILED", 
                             f"Made {calls_made} calls without rate limit error")
                             
        except RateLimitExceeded as e:
            elapsed = time.time() - start
            self.report_result("rate_limits", "basic_enforcement", "PASSED", 
                             f"Rate limit hit after {calls_made} calls in {elapsed:.2f}s: {e}")
        except Exception as e:
            self.report_result("rate_limits", "basic_enforcement", "ERROR", 
                             f"Unexpected error: {type(e).__name__}: {e}")
        ...
        try:
            ...
            if elapsed < 2.0:
                self.report_result("rate_limits", "async_burst", "FAILED", 
                                 f"20 calls completed in {elapsed:.2f}s (should take >2s)")
            else:
                self.report_result("rate_limits", "async_burst", "PASSED", 
                                 f"Properly rate limited: 20 calls in {elapsed:.2f}s")
                                 
        except asyncio.TimeoutError:
            self.report_result("rate_limits", "async_burst", "WARNING", 
                             "Test timed out - rate limiter may be blocking indefinitely")
        except Exception as e:
            self.report_result("rate_limits", "async_burst", "ERROR", 
                             f"Unexpected error: {type(e).__name__}: {e}")
        ...
        try:
            ...
            self.report_result("rate_limits", "context_manager", "FAILED", 
                             f"Made {successes} calls without rate limit")
                             
        except RateLimitExceeded as e:
            self.report_result("rate_limits", "context_manager", "PASSED", 
                             f"Context manager rate limit after {successes} calls: {e}")
        except Exception as e:
            self.report_result("rate_limits", "context_manager", "ERROR", 
                             f"Unexpected error: {type(e).__name__}: {e}")
    ...
    def test_timeout_limits(self):
        ...
        try:
            ...
            self.report_result("timeout_limits", "basic_signal", "FAILED", 
                             f"Operation completed: {result}")
                             
        except TimeoutError as e:
            self.report_result("timeout_limits", "basic_signal", "PASSED", 
                             f"Correctly timed out: {e}")
        except Exception as e:
            self.report_result("timeout_limits", "basic_signal", "ERROR", 
                             f"Unexpected error: {type(e).__name__}: {e}")
        ...
        try:
            with timeout(1):  # Outer timeout shorter
                result = nested_slow()
            self.report_result("timeout_limits", "nested_timeouts", "FAILED", 
                             f"Operation completed: {result}")
                             
        except TimeoutError as e:
            self.report_result("timeout_limits", "nested_timeouts", "PASSED", 
                             f"Outer timeout triggered: {e}")
        except Exception as e:
            self.report_result("timeout_limits", "nested_timeouts", "ERROR", 
                             f"Unexpected error: {type(e).__name__}: {e}")
        ...
        try:
            ...
            self.report_result("timeout_limits", "decorator", "FAILED", 
                             f"Decorated function completed: {result}")
                             
        except TimeoutError as e:
            self.report_result("timeout_limits", "decorator", "PASSED", 
                             f"Decorator timeout worked: {e}")
        except Exception as e:
            self.report_result("timeout_limits", "decorator", "ERROR", 
                             f"Unexpected error: {type(e).__name__}: {e}")
...
"""
            ...
            try:
                ...
                self.report_result("file_size_limits", "simple_parser", "FAILED", 
                ...
                if "file size" in str(e).lower() or "too large" in str(e).lower():
                    self.report_result("file_size_limits", "simple_parser", "PASSED", 
                                     f"File size limit enforced: {e}")
                else:
                    self.report_result("file_size_limits", "simple_parser", "WARNING", 
            ...
            try:
                ...
                self.report_result("file_size_limits", "config_validation", "FAILED", 
                                 "Accepted negative file size limit")
            except ValueError as e:
                self.report_result("file_size_limits", "config_validation", "PASSED", 
                                 f"Config validation worked: {e}")
            except Exception as e:
                self.report_result("file_size_limits", "config_validation", "ERROR", 
                                 f"Unexpected error: {type(e).__name__}: {e}")
            ...
            try:
                ...
                self.report_result("file_size_limits", "under_limit", "PASSED", 
                                 f"Successfully parsed file under limit ({len(under_content)/1024:.1f}KB)")
            except Exception as e:
                self.report_result("file_size_limits", "under_limit", "FAILED", 
    ...
    def test_batch_limits(self):
        ...
        if hasattr(BatchExporter, 'MAX_BATCH_SIZE'):
            ...
            self.report_result("batch_limits", "constant_exists", "PASSED", 
                             f"MAX_BATCH_SIZE = {limit}")
        else:
            self.report_result("batch_limits", "constant_exists", "FAILED", 
        ...
        if hasattr(BatchExporter, 'MAX_CONCURRENT_EXPORTS'):
            ...
            self.report_result("batch_limits", "concurrent_limit", "PASSED", 
                             f"MAX_CONCURRENT_EXPORTS = {limit}")
        else:
            self.report_result("batch_limits", "concurrent_limit", "WARNING", 
    ...
    async def test_lsp_client_limits(self):
        ...
        try:
            client = LeanLSPClient(timeout=1.0, max_memory_mb=100, 
                                 rate_limit_calls=5, rate_limit_window=1.0)
            ...
            with tempfile.NamedTemporaryFile(suffix=".lean", mode='w') as f:
                ...
                if elapsed > 1.5:
                    self.report_result("lsp_client_limits", "timeout", "WARNING", 
                                     f"Operation took {elapsed:.2f}s with 1s timeout")
                else:
                    self.report_result("lsp_client_limits", "timeout", "INFO", 
                                     f"Operation completed in {elapsed:.2f}s")
                    
        except Exception as e:
            if "timeout" in str(e).lower():
                self.report_result("lsp_client_limits", "timeout", "PASSED", 
                                 f"LSP timeout enforced: {e}")
            else:
                self.report_result("lsp_client_limits", "timeout", "ERROR", 
                                 f"Unexpected error: {type(e).__name__}: {e}")
        ...
        try:
            client = LeanLSPClient(timeout=30.0, rate_limit_calls=3, rate_limit_window=1.0)
            ...
            with tempfile.NamedTemporaryFile(suffix=".lean", mode='w') as f:
                ...
                try:
                    ...
                    self.report_result("lsp_client_limits", "rate_limit", "FAILED", 
                                     f"Made {calls_made} calls without rate limit")
                                     
                except RateLimitExceeded as e:
                    self.report_result("lsp_client_limits", "rate_limit", "PASSED", 
                                     f"LSP rate limit after {calls_made} calls: {e}")
                    
        except Exception as e:
            self.report_result("lsp_client_limits", "rate_limit", "ERROR", 
                             f"Unexpected error: {type(e).__name__}: {e}")
    ...
    async def stress_test_all(self):
        ...
        if total > 0:
            ...
            enforcement_score = (passed / (passed + failed)) * 100 if (passed + failed) > 0 else 0
        else:
            robustness_score = 0
            enforcement_score = 0
        ...
        for category, tests in self.results.items():
            ...
            for test, result in tests.items():
                if result["status"] == "PASSED":
                    findings["working_limits"].append(f"{category}.{test}: {result['details']}")
                elif result["status"] == "FAILED":
                    findings["bypassed_limits"].append(f"{category}.{test}: {result['details']}")
                elif result["status"] == "WARNING":
                    findings["performance_issues"].append(f"{category}.{test}: {result['details']}")
                elif result["status"] == "ERROR":
                    findings["crashes"].append(f"{category}.{test}: {result['details']}")
        
        # Add recommendations based on findings
        if not findings["working_limits"]:
            findings["recommendations"].append("CRITICAL: No resource limits are properly enforced!")
        
        if findings["bypassed_limits"]:
            findings["recommendations"].append("Fix bypassed limits to prevent resource exhaustion")
            
        if findings["crashes"]:
            findings["recommendations"].append("Address crash-causing errors for stability")
            
        if "memory_limits.psutil_available" in str(findings["performance_issues"]):
            findings["recommendations"].append("Install psutil for memory monitoring: pip install psutil")
