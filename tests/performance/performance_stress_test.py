...
from proof_sketcher.core.resource_limits import RateLimiter, ResourceMonitor, timeout
...
print(
    f"   Allocated {len(allocations)} chunks over {allocation_times[-1] if allocation_times else 0:.1f}s"
)
...
with tempfile.TemporaryDirectory() as tmpdir:
    for size_mb in file_sizes:
        ...
        if result.success:
            ...
            print(
                f"   {size_mb}MB: {parse_time:.3f}s ({len(result.theorems)} theorems)"
            )
        else:
...
async def concurrent_stress():
    ...
    results = await asyncio.gather(
        *[worker(i) for i in range(5)], return_exceptions=True
    )
