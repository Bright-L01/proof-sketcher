...
class ConcurrentSimulator:
    ...
    def simulate_student_session(self, student_id: int, session_duration: int = 30) -> StudentResult:
            ...
            for test_file in self.test_files:
                ...
                try:
                    ...
                    if result.success:
                        ...
                        for i, theorem in enumerate(result.theorems[:5]):  # Student works on 5 theorems
                            total_operations += 1
                            try:
        ...
        print(f"Student {student_id}: Completed session - {theorems_processed} theorems, {success_rate:.1%} success")
    ...
    def run_classroom_simulation(self, num_students: int, session_duration: int = 30) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=min(num_students, 20)) as executor:
            ...
            futures = [
                executor.submit(self.simulate_student_session, student_id, session_duration)
        ...
        if student_results:
            ...
            avg_success_rate = statistics.mean([r.success_rate for r in student_results])
            avg_theorems_per_student = statistics.mean([r.theorems_processed for r in student_results])
...
def main():
    ...
    for num_students, duration in test_scenarios:
        ...
        try:
            ...
            print(f"  System throughput: {result['system_throughput_theorems_per_second']:.2f} theorems/sec")
    ...
    if successful_tests:
        max_successful_students = max(r["num_students"] for r in successful_tests 
                                    if r["avg_success_rate"] >= 0.8)
        best_throughput = max(r["system_throughput_theorems_per_second"] for r in successful_tests)
        max_memory_increase = max(r["memory_increase_mb"] for r in successful_tests)
        
        print(f"✅ Maximum concurrent students (80%+ success): {max_successful_students}")
