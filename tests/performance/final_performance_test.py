...
class UniversityPerformanceTester:
            ...
            try:
                ...
                for theorem in theorems:
                    try:
                        ...
                        if sketch and sketch.intuitive_overview:  # Check for valid content
    ...
    def simulate_student_workload(self, student_id: int, duration: int = 20) -> PerformanceData:
                    ...
                    if result.success:
                        ...
                        for theorem in result.theorems[:3]:
                            try:
                                ...
                                errors.append(f"Student {student_id} generation error: {e}")
    ...
    def test_concurrent_classroom(self, num_students: int, session_duration: int = 20) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=min(num_students, 25)) as executor:
            futures = [
                executor.submit(self.simulate_student_workload, student_id, session_duration)
        ...
        avg_theorems_per_student = statistics.mean([r.items_processed for r in student_results]) if student_results else 0
        avg_student_time = statistics.mean([r.duration for r in student_results]) if student_results else 0
        system_throughput = total_theorems / (overall_end - overall_start) if (overall_end - overall_start) > 0 else 0
        ...
        print(f"  Successful students: {len(successful_students)}/{num_students} ({success_rate:.1%})")
    ...
    def run_scalability_test(self) -> Dict[str, Any]:
        ...
        for size in test_sizes:
            ...
            result = self.test_concurrent_classroom(size, 15)  # Shorter sessions for scalability test
            scalability_results.append(result)
            
            # Stop if we hit a failure threshold
            if result["success_rate"] < 0.5:
                print(f"⚠️ Stopping scalability test - success rate too low at {size} students")
                break
        
        # Find maximum viable capacity
        successful_tests = [r for r in scalability_results if r["status"] == "PASSED"]
        max_capacity = max([r["num_students"] for r in successful_tests]) if successful_tests else 0
    ...
    def assess_university_readiness(self, single_file_results: Dict, scalability_results: Dict) -> Dict[str, Any]:
        ...
        if successful_single_tests:
            avg_single_throughput = statistics.mean([r.throughput for r in successful_single_tests])
        ...
        if (assessment["classroom_ready"] and 
            len(assessment["bottlenecks"]) <= 2 and
            scalability_data["tests_passed"] >= scalability_data["tests_total"] * 0.7):
            assessment["production_ready"] = True
        
        # Overall grading
        score = 0
        if assessment["production_ready"]: score += 40
        if assessment["university_scale_ready"]: score += 30
        if assessment["classroom_ready"]: score += 20
        if len(assessment["bottlenecks"]) == 0: score += 10
        
        if score >= 90:
            assessment["overall_grade"] = "A - Excellent, ready for large-scale deployment"
        elif score >= 80:
            assessment["overall_grade"] = "B - Good, suitable for production with monitoring"
        elif score >= 60:
        ...
        if not assessment["production_ready"]:
            assessment["recommendations"].append("Address performance bottlenecks before production")
        if not assessment["university_scale_ready"]:
            assessment["recommendations"].append("Optimize concurrent processing for larger classes")
        if "Low single-file throughput" in assessment["bottlenecks"]:
            assessment["recommendations"].append("Optimize proof sketch generation algorithms")
        if "High memory usage per file" in assessment["bottlenecks"]:
            assessment["recommendations"].append("Implement memory optimization and garbage collection")
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        assessment = self.assess_university_readiness(single_file_results, scalability_results)
    ...
    def print_final_report(self, report: Dict[str, Any]):
        ...
        print(f"   University Scale: {'✅ YES' if assessment['university_scale_ready'] else '❌ NO'}")
        print(f"   Classroom Ready:  {'✅ YES' if assessment['classroom_ready'] else '❌ NO'}")
        
        capacity = assessment["capacity_analysis"]
        print(f"\n📈 CAPACITY ANALYSIS:")
        print(f"   Maximum Concurrent Students: {capacity['max_concurrent_students']}")
        print(f"   Scalability Tests Passed: {capacity['scalability_tests_passed']}/{capacity['scalability_tests_total']}")
        
        performance = assessment["performance_analysis"]
        if performance:
            print(f"\n⚡ PERFORMANCE METRICS:")
            print(f"   Average Throughput: {performance['avg_single_file_throughput']:.2f} theorems/sec")
            print(f"   Memory Usage: {performance['max_single_file_memory_mb']:.1f}MB per file")
