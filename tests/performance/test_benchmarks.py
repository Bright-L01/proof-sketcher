...
from proof_sketcher.optimizations.performance import PerformanceOptimizer, BatchConfig, WorkloadMetrics
...
from proof_sketcher.exporter.models import ExportOptions, ExportContext
...
class BenchmarkSuite:
        ...
        return [
            {
                ...
                'proof_steps': [
                    f'Step {j}: Apply theorem T{j}' 
                    for j in range(1, min(i + 2, 6))
                ],
    ...
    def benchmark_notation_handler(self, iterations: int = 1000) -> Dict[str, Any]:
        ...
        print(f"   Baseline: {baseline_time:.3f}s ({results['baseline_throughput']:.0f} ops/sec)")
        print(f"   Optimized: {optimized_time:.3f}s ({results['optimized_throughput']:.0f} ops/sec)")
        print(f"   Speedup: {speedup:.2f}x ({results['improvement_percent']:.1f}% improvement)")
    ...
    def benchmark_caching_system(self, cache_size: int = 1000) -> Dict[str, Any]:
        ...
        print(f"   Write: {write_time:.3f}s ({results['write_throughput']:.0f} ops/sec)")
        print(f"   Read (hits): {read_hit_time:.3f}s ({results['read_hit_throughput']:.0f} ops/sec)")
        print(f"   Read (misses): {read_miss_time:.3f}s ({results['read_miss_throughput']:.0f} ops/sec)")
    ...
    def benchmark_parallel_processing(self, num_theorems: int = 50) -> Dict[str, Any]:
        ...
        print(f"   Sequential: {sequential_time:.3f}s ({results['sequential_throughput']:.2f} theorems/sec)")
        print(f"   Parallel: {parallel_time:.3f}s ({results['parallel_throughput']:.2f} theorems/sec)")
    ...
    def benchmark_memory_usage(self) -> Dict[str, Any]:
        ...
        results = {
            ...
            'memory_efficiency_percent': (memory_savings / memory_no_cache) * 100 if memory_no_cache > 0 else 0,
            'dataset_size': len(large_theorems)
        }
        
        print(f"   Without caching: {memory_no_cache:.1f}MB")
        print(f"   With caching: {memory_with_cache:.1f}MB")
        print(f"   Memory savings: {memory_savings:.1f}MB ({results['memory_efficiency_percent']:.1f}%)")
    ...
    def benchmark_resource_limits(self) -> Dict[str, Any]:
        ...
        print(f"   Timeout limiting: {'✅' if timeout_test_passed else '❌' if timeout_test_passed is not None else '⚠️ Unsupported'}")
        print(f"   Animation limiting: {'✅' if animation_limits_applied else '❌'}")
        print(f"   Resource monitoring: {'✅' if results['monitoring_functional'] else '❌'}")
    ...
    def _calculate_performance_score(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'caching_system' in results and 'cache_stats' in results['caching_system']:
            hit_rate = results['caching_system']['cache_stats'].get('hit_rate_percent', 0)
        ...
        if 'memory_usage' in results and 'memory_efficiency_percent' in results['memory_usage']:
    ...
    def _print_summary(self, results: Dict[str, Any]):
        ...
        if 'overall_performance_score' in results:
            ...
            print(f"🎯 Overall Performance Score: {score_data['overall_score']:.1f}/100 (Grade: {score_data['grade']})")
            print()
...
def test_performance_improvements():
    ...
    if 'notation_handler' in results and 'speedup' in results['notation_handler']:
        speedup = results['notation_handler']['speedup']
        assert speedup > 3.0, f"Notation handler only {speedup:.2f}x faster (target: 3x)"
    
    # Verify parallel processing improvement
    if 'parallel_processing' in results and 'speedup' in results['parallel_processing']:
        speedup = results['parallel_processing']['speedup']
        assert speedup > 2.0, f"Parallel processing only {speedup:.2f}x faster (target: 2x)"
    
    # Verify memory efficiency
    if 'memory_usage' in results and 'memory_efficiency_percent' in results['memory_usage']:
        efficiency = results['memory_usage']['memory_efficiency_percent']
        assert efficiency > 10.0, f"Memory efficiency only {efficiency:.1f}% (target: 10%)"
...
def test_resource_limits_functionality():
    ...
    assert results['animation_limits_applied'], "Animation complexity limits not applied"
