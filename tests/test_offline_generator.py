...
from src.proof_sketcher.generator.offline import (
    OfflineGenerator, OfflineTemplates
)
from src.proof_sketcher.generator.models import (
    ProofSketch, ProofStep, GenerationConfig, GenerationRequest, GenerationResponse
)
...
class TestOfflineGenerator:
  ...
  · apply prime_of_coprime
    ...
    def test_analyze_theorem_type_equality(self, generator):
        ...
        theorem = TheoremInfo(
            name="test_eq",
            statement="theorem test_eq : 2 + 2 = 4",
            proof="rfl"
        )
    ...
    def test_analyze_theorem_type_implication(self, generator):
        ...
        theorem = TheoremInfo(
            name="impl_test",
            statement="theorem impl_test (h : P) : Q",
            proof="exact h"
        )
    ...
    def test_generate_introduction_without_docstring(self, generator):
        ...
        theorem = TheoremInfo(
            name="undocumented",
            statement="theorem undocumented : a = b",
            proof="sorry"
        )
        
        intro = generator._generate_introduction(theorem, "equality", ["natural_numbers"])
    ...
    def test_generate_proof_sketch_minimal_theorem(self, generator):
        ...
        theorem = TheoremInfo(
            name="minimal",
            statement="theorem minimal : True",
            proof="trivial"
        )
    ...
    def test_handle_axiom(self, generator):
        ...
        assert "assumed" in sketch.introduction.lower() or "postulated" in sketch.introduction.lower()
        assert len(sketch.key_steps) >= 1
    ...
    def test_empty_proof_handling(self, generator):
        ...
        assert "deferred" in sketch.key_steps[0].description or "incomplete" in sketch.key_steps[0].description
    ...
    def test_special_tactics_handling(self, generator):
        ...
        assert len(sketch.key_steps) > 0
        assert "decision" in sketch.key_steps[0].description.lower() or "decidable" in sketch.key_steps[0].description.lower()
...
class TestOfflineGeneratorIntegration:
  ...
  | cons x xs ih =>
    simp [List.append, ih]""",
        ...
        induction_steps = [s for s in sketch.key_steps if any("induction" in t for t in s.tactics)]
        assert len(induction_steps) > 0
        
        # Check mathematical areas
        assert "Data structures" in sketch.mathematical_areas or "Lists" in sketch.mathematical_areas
        ...
        assert "established" in sketch.conclusion.lower() or "proven" in sketch.conclusion.lower()
