...
class TestSafeSerialization:
    ...
    def test_malicious_json_rejected(self, tmp_path):
        ...
        malicious_json = (
            '{"__class__": "os.system", "__module__": "os", "cmd": "rm -rf /"}'
        )
...
class TestSecurityValidators:
    ...
    def test_url_validation(self):
        ...
        safe_urls = ["https://example.com/api", "http://public-api.com/data"]
...
class TestInputSanitizers:
    ...
    def test_json_input_sanitization(self):
        ...
        nested_data = {
            "safe": "value",
            "nested": {"deep": ["item1", "item2"], "text": "<script>alert(1)</script>"},
        }
...
class TestCacheManagerSecurityFixes:
    ...
    def test_cache_uses_json_serialization(self, tmp_path):
        ...
        cache_manager = BatchCacheManager(
            cache_dir=tmp_path, ttl_hours=1, max_cache_size_gb=1
        )
...
def test_no_pickle_imports():
    ...
    main_attrs = [
        attr for attr in dir(safe_ser.SafeSerializer) if not attr.startswith("_")
    ]
