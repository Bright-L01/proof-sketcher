...
from proof_sketcher.core.exceptions import InputValidationError, ProofSketcherError
from proof_sketcher.core.exceptions import ValidationError as PSValidationError
...
class TestModelsProperties:
    ...
    def test_theorem_info_validation(self, name, statement, proof, line_number):
        ...
        theorem = TheoremInfo(
            name=name, statement=statement, proof=proof, line_number=line_number
        )
    ...
    def test_parse_error_creation(self, message, line_number, column):
        ...
        error = ParseError(message=message, line_number=line_number, column=column)
    ...
    @given(
        ...
        difficulty=st.sampled_from(
            ["trivial", "easy", "intermediate", "hard", "expert"]
        ),
    )
    def test_proof_sketch_validation(
        self, theorem_name, theorem_statement, introduction, conclusion, difficulty
    ):
...
class TestExceptionProperties:
    ...
    @given(
        ...
        error_code=st.text(
            alphabet=string.ascii_uppercase + "_", min_size=3, max_size=20
        ),
    )
...
class CacheStateMachine(RuleBasedStateMachine):
    ...
    @rule(key=st.text(min_size=1, max_size=20), value=st.integers(), target=keys)
        ...
        st.dictionaries(
            ...
            st.recursive(
                ...
                lambda children: st.dictionaries(
                    st.text(max_size=10), children, max_size=3
                ),
                max_leaves=10,
            ),
            max_size=5,
...
class TestPropertyInvariants:
    ...
    def test_parse_result_invariants(self, data):
        ...
        theorems = data.draw(
            st.lists(
                st.builds(
                    TheoremInfo, name=theorem_names(), statement=lean_expressions()
                ),
                max_size=5,
            )
        )
        errors = data.draw(
            st.lists(st.builds(ParseError, message=st.text(min_size=1)), max_size=5)
        )

        success = data.draw(st.booleans())

        result = ParseResult(theorems=theorems, errors=errors, success=success)
