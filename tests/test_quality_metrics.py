...
class TestMetrics:
    ...
    def assertion_density(self) -> float:
        ...
        return (
            self.assertion_count / self.lines_of_code if self.lines_of_code > 0 else 0.0
        )
...
class ProjectMetrics:
    ...
    def average_quality_score(self) -> float:
        ...
        if not self.files:
            return 0.0
        return sum(f.average_quality_score for f in self.files.values()) / len(
            self.files
        )
...
class TestQualityAnalyzer:
    ...
    def _analyze_test_function(
        self, node: ast.FunctionDef, filename: str
    ) -> TestMetrics:
    ...
    def generate_report(self, output_path: Optional[Path] = None) -> str:
        ...
        for category, count in sorted(
            self.metrics.test_distribution.items(), reverse=True
        ):
            percentage = (
                (count / self.metrics.total_tests * 100)
