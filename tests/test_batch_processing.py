...
""")
    ...
    def test_scan_project_basic(self, sample_project):
        ...
        all_theorems = sum(len(theorems) for theorems in result["file_theorems"].values())
...
class TestCacheManager:
    ...
    def test_cache_statistics(self, cache_manager):
        ...
        # Add some entries
        for i in range(5):
            key = cache_manager.get_cache_key(f"file{i}.lean", f"theorem{i}", f"content{i}")
    ...
    def test_cache_thread_safety(self, cache_manager):
        ...
        def cache_operation(thread_id):
            try:
                for i in range(10):
                    key = cache_manager.get_cache_key(f"thread{thread_id}", f"item{i}", "content")
