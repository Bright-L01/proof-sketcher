...
from proof_sketcher.animator.models import AnimationConfig, ManimConfig, AnimationStyle, AnimationQuality
            ...
            try:
                ...
                if isinstance(response, dict):
                    if response.get('error'):
                        logger.warning(f"⚠️ Animation completed with fallback: {response['error']}")
...
async def demo_fallback_path():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        async with ProductionAnimator(
            ...
            logger.info("⚠️ Manim server unavailable - expecting fallback to static images")
            ...
            try:
                ...
                if isinstance(response, dict):
                    if response.get('error'):
                        logger.info(f"✅ Fallback completed as expected: {response['error']}")
                        logger.info(f"📊 Metadata: {response.get('metadata', {})}")
                        ...
                        if 'fallback_reason' in metadata:
                            logger.info(f"🔄 Fallback reason: {metadata['fallback_reason']}")
                    else:
...
async def demo_error_scenarios():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        async with ProductionAnimator(
            ...
            test_cases = [
                ("Invalid theorem name", ProofSketch(
            ...
            for case_name, theorem in test_cases:
                ...
                try:
                    ...
                    if isinstance(response, dict) and response.get('error'):
                        logger.info(f"✅ Error handled gracefully: {response['error'][:50]}...")
                        logger.info(f"🛡️ Error type: {response.get('metadata', {}).get('error_type', 'unknown')}")
                    else:
