...
Demo will process example files and generate publication-ready documentation.
    ...
    def demonstrate_enhanced_parser(self):
        ...
        self.console.print(
            "[bold cyan]🔍 PHASE 1: Enhanced Parser Demonstration[/bold cyan]\n"
        )
        ...
        improvement = (
            ((enhanced_count - basic_count) / basic_count * 100)
            if basic_count > 0
            else 0
        )
    ...
    def demonstrate_generation_and_export(self):
        ...
        if sketches:
            ...
            html_context = ExportContext(
                format=ExportFormat.HTML, output_dir=html_dir, sketches=sketches
            )
            ...
            for sketch in sketches:
                try:
                    ...
                    self.console.print(
                        f"  HTML export failed for {sketch.theorem_name}: {e}"
                    )
            ...
            md_context = ExportContext(
                format=ExportFormat.MARKDOWN, output_dir=md_dir, sketches=sketches
            )
            ...
            for sketch in sketches:
                try:
                    ...
                    self.console.print(
                        f"  Markdown export failed for {sketch.theorem_name}: {e}"
                    )
            ...
            for format_name, success_count in export_results.items():
                ...
                export_table.add_row(
                    format_name, f"{success_count}/{len(sketches)}", str(output_path)
                )
    ...
    def demonstrate_performance_analysis(self):
        ...
        performance_table.add_row(
            "File size", f"{mathlib_file.stat().st_size / 1024:.1f}", "KB"
        )
        ...
        performance_table.add_row(
            "Enhanced parse time", f"{avg_enhanced_time:.0f}", "ms"
        )

        if theorem_count > 0:
            time_per_theorem = avg_parse_time / theorem_count
            performance_table.add_row(
                "Time per theorem", f"{time_per_theorem:.1f}", "ms"
            )

            theorems_per_second = (theorem_count * 1000) / avg_parse_time
            performance_table.add_row(
                "Processing rate", f"{theorems_per_second:.1f}", "theorems/sec"
            )
        ...
        return {
            ...
            "theorems_per_second": (
                (theorem_count * 1000) / avg_parse_time if theorem_count > 0 else 0
            ),
        }

    def show_demo_summary(
        self, parser_stats, sketches, export_results, batch_stats, performance_stats
    ):
        ...
        # Parser capabilities
        total_constructs = (
            parser_stats.get("total_constructs", 0) if parser_stats else 0
        )
        summary_table.add_row(
            "Enhanced Parser", "Language constructs found", str(total_constructs)
        )
        ...
        if batch_stats:
            summary_table.add_row(
                "Batch Processing", "Files processed", str(batch_stats.total_files)
            )
            summary_table.add_row(
                "",
                "Success rate",
                f"{(batch_stats.successful_files/batch_stats.total_files*100):.1f}%",
            )
        ...
        if performance_stats:
            ...
            summary_table.add_row(
                "Performance", "Processing rate", f"{rate:.1f} thm/sec"
            )
    ...
    def run_demo(self):
        ...
        try:
            ...
            self.show_demo_summary(
                parser_stats, sketches, export_results, batch_stats, performance_stats
            )
