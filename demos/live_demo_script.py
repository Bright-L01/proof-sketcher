...
Mode: {'Interactive' if self.interactive else 'Automated'} | {'Quick' if self.quick_mode else 'Full'}
    ...
    def demo_1_enhanced_parsing(self):
        ...
        if self.interactive:
            self.console.print(
                "This demo shows how Proof Sketcher handles complex Lean 4 constructs."
            )
            self.console.print(
                "We'll analyze a real Mathlib-style file with theorems, definitions, structures, and more.\\n"
            )
        ...
        test_file = (
            Path(__file__).parent.parent / "examples" / "mathlib_real_world.lean"
        )

        if not test_file.exists():
            self.console.print(
                "[red]❌ Test file not found - creating demo content...[/red]"
            )
        ...
        with Progress(
            ...
            task = progress.add_task(
                "Parsing with enhanced capabilities...", total=None
            )
    ...
    def demo_2_generation_and_export(self):
        ...
        if self.interactive:
            self.console.print(
                "Now we'll generate natural language explanations and export to multiple formats."
            )
            self.console.print(
                "This showcases how formal proofs become accessible explanations.\\n"
            )
        ...
        test_file = (
            ...
            / "examples"
            / "classical"
            / "simple_examples.lean"
        )

        if not test_file.exists():
            self.console.print(
                "[red]❌ Simple examples not found - creating demo content...[/red]"
            )
        ...
        demo_theorems = (
            result.theorems[:3] if not self.quick_mode else result.theorems[:1]
        )
        self.console.print(
        ...
        with Progress(
            ...
            gen_task = progress.add_task(
                "Generating explanations...", total=len(demo_theorems)
            )
...
{sample_sketch.mathematical_context or 'N/A'}""",
    ...
    def demo_3_batch_processing(self):
        ...
        if self.interactive:
            self.console.print(
                "This demo shows how Proof Sketcher handles multiple files efficiently."
            )
            self.console.print(
                "We'll process several mathematical files in parallel with performance monitoring.\\n"
            )
        ...
        if not processor.jobs:
            self.console.print(
                "[yellow]⚠️  No Lean files found for batch processing[/yellow]"
            )
            return

        file_count = len(processor.jobs)
        self.console.print(
            f"[green]Found {file_count} files for batch processing[/green]"
        )
        ...
        # Performance summary
        perf_table = Table(
            title="Batch Processing Performance", title_style="bold green"
        )
        ...
        perf_table.add_row(
            "Success rate", f"{(stats.successful_files/stats.total_files*100):.1f}", "%"
        )
        perf_table.add_row("Total time", f"{batch_time:.0f}", "ms")
        perf_table.add_row(
            "Average per file", f"{batch_time/file_count:.0f}", "ms/file"
        )
        ...
        if stats.total_theorems > 0:
            ...
            perf_table.add_row(
                "Processing rate",
                f"{(stats.total_theorems * 1000 / batch_time):.1f}",
                "thm/sec",
            )
        ...
        self.results["batch"] = {
            ...
            "processing_rate": (
                (stats.total_theorems * 1000 / batch_time) if batch_time > 0 else 0
            ),
        }
    ...
    def demo_4_performance_analysis(self):
        ...
        if self.interactive:
            self.console.print(
                "Let's analyze Proof Sketcher's performance characteristics."
            )
            self.console.print(
                "We'll test parsing speed, memory usage, and scaling behavior.\\n"
            )
        ...
        with Progress(
            ...
            perf_task = progress.add_task(
                "Running performance tests...", total=len(test_files)
            )
                ...
                performance_results.append(
                    {
                        ...
                        "theorems_per_second": (
                            (theorem_count * 1000 / avg_parse_time)
                            if avg_parse_time > 0
                            else 0
                        ),
                    }
        ...
        perf_table = Table(
            title="Performance Analysis Results", title_style="bold green"
        )
        ...
        avg_rate = sum(r["theorems_per_second"] for r in performance_results) / len(
            performance_results
        )
        ...
        if avg_rate > 10.0:
            self.console.print(
                "  ✅ [green]Excellent performance - suitable for large-scale processing[/green]"
            )
        elif avg_rate > 2.0:
            self.console.print(
                "  ✅ [green]Good performance - suitable for regular use[/green]"
            )
        else:
            self.console.print(
                "  ⚠️ [yellow]Moderate performance - consider optimization for large files[/yellow]"
            )
    ...
    def show_demo_summary(self):
        ...
        if "parsing" in self.results:
            ...
            summary_table.add_row(
                "Enhanced Parser",
                "Constructs found",
                str(parsing.get("total_constructs", 0)),
            )
            summary_table.add_row(
                "", "Parse time", f"{parsing.get('parse_time_ms', 0):.0f} ms"
            )
        ...
        if "generation" in self.results:
            ...
            summary_table.add_row(
                "Generation",
                "Theorems processed",
                str(generation.get("theorems_processed", 0)),
            )
            summary_table.add_row(
                "", "Export formats", str(generation.get("export_formats", 0))
            )
        ...
        if "batch" in self.results:
            ...
            summary_table.add_row(
                "Batch Processing",
                "Files processed",
                str(batch.get("files_processed", 0)),
            )
            summary_table.add_row(
                "", "Success rate", f"{batch.get('success_rate', 0)*100:.1f}%"
            )
        ...
        if "performance" in self.results:
            ...
            summary_table.add_row(
                "Performance",
                "Average rate",
                f"{performance.get('average_rate', 0):.1f} thm/sec",
            )
        ...
        self.console.print(
            f"  📝 Markdown exports: [cyan]{self.demo_dir / 'markdown'}[/cyan]"
        )
        self.console.print(
            f"  📦 Batch outputs: [cyan]{self.demo_dir / 'batch_output'}[/cyan]"
        )
...
@click.command()
@click.option(
    "--interactive/--automated", default=True, help="Interactive or automated mode"
)
@click.option("--quick", is_flag=True, help="Quick demo mode (shorter)")
@click.option(
    "--save-results", type=click.Path(), help="Save demo results to JSON file"
)
