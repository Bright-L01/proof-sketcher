...
class QualityFixer:
    ...
    def _remove_unused_imports(self, content: str) -> Tuple[str, int]:
        ...
        for node, name, asname in imports:
            ...
            if (check_name not in used_names and 
                check_name in ['typing.Optional'] and  # Only remove specific safe imports
                hasattr(node, 'lineno')):
                lines_to_remove.add(node.lineno - 1)
                changes += 1
        
        if lines_to_remove:
            fixed_lines = [line for i, line in enumerate(lines) 
...
def main():
    ...
    parser.add_argument(
        "--module", 
        help="Specific module to fix (e.g., 'parser', 'generator')"
    )
