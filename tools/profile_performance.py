...
class PerformanceProfiler:
    ...
    def profile_theorem_processing(self, theorem_file: Path, theorem_name: str = None) -> PerformanceMetrics:
    ...
    def profile_batch_processing(self, project_dir: Path, max_theorems: int = 10) -> PerformanceMetrics:
    ...
    def _process_single_theorem(self, theorem_file: Path, theorem_name: str = None) -> Dict:
        ...
        # Select theorem
        theorem = theorems[0] if not theorem_name else next(
            (t for t in theorems if t.name == theorem_name), theorems[0]
        )
    ...
    def _identify_bottlenecks(self, stats: pstats.Stats) -> List[Dict[str, Any]]:
        ...
        for func_info, profile in sorted_stats:
            ...
            per_call_time = profile.cumtime / profile.ncalls if profile.ncalls > 0 else 0
            ...
            if profile.cumtime > 0.05 or per_call_time > 0.01:
                bottlenecks.append({
                    ...
                    'percentage': (profile.cumtime / stats.total_tt) * 100 if stats.total_tt > 0 else 0
                })
    ...
    def _save_metrics(self, metrics: PerformanceMetrics):
        ...
        try:
            ...
            profile_file = self.output_dir / f"{metrics.operation_name}_{timestamp}_profile.txt"
    ...
    def compare_with_baseline(self, metrics: PerformanceMetrics) -> Dict[str, Any]:
        ...
        Returns:
            Comparison report
        """
        baseline_key = metrics.operation_name.split('_')[0]  # e.g., "theorem_processing"
        ...
        time_regression = ((metrics.execution_time - baseline_time) / baseline_time) * 100
        memory_regression = ((metrics.memory_peak_mb - baseline_memory) / baseline_memory) * 100
    ...
    def generate_performance_report(self) -> str:
        ...
        if self.system_metrics.load_average:
            ...
            report_lines.append(f"- Load Average: {load_1:.2f}, {load_5:.2f}, {load_15:.2f}")
        ...
        if self.baseline_metrics:
            ...
            for key, value in self.baseline_metrics.items():
                if key.endswith('_time'):
                    ...
                    report_lines.append(f"- {operation} memory baseline: {value:.1f} MB")
...
def main():
        ...
        for test_file in existing_files[:1]:  # Profile first file
            try:
                ...
                if comparison['status'] != 'baseline_set':
                    print(f"   Time Change: {comparison['time_regression_percent']:+.1f}%")
                    print(f"   Memory Change: {comparison['memory_regression_percent']:+.1f}%")
    ...
    print(f"   System: {profiler.system_metrics.cpu_count} cores, {profiler.system_metrics.total_memory_gb:.1f}GB RAM")
