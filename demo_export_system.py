...
def demo_multi_format_consistency():
    ...
    if successful_formats >= 2:
        ...
        for format_name, files in formats_exported.items():
            if files:
                ...
                try:
                    ...
                    step_mentions = sum(
                        1 for step in sketch.key_steps if step.description in content
                    )
        ...
        print(
            f"🔢 Proof steps consistency: {step_count_consistent}/{successful_formats}"
        )
...
def main():
    ...
    try:
        ...
        if static_path:
            ...
            print(
                f"✅ Animation script generation: {'WORKING' if animation_script else 'FAILED'}"
            )
            print(f"✅ Static visualization: {'WORKING' if static_path else 'FAILED'}")
            print(
                f"✅ HTML export: {'WORKING' if export_results.get('HTML') else 'FAILED'}"
            )
            print(
                f"✅ Markdown export: {'WORKING' if export_results.get('Markdown') else 'FAILED'}"
            )
            print(
                f"✅ PDF export: {'WORKING' if export_results.get('PDF') else 'SKIPPED'}"
            )
            ...
            if success_count >= 2:
                print(
                    "✨ MILESTONE 2.2 COMPLETE: Multi-format export system operational!"
                )
            else:
